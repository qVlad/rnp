...
> ⚠️ **Источники по периоду (TASK-LEAD-153/157):**
>
> Hybrid 3-tier для заказов и выкупов:
>
> 1. **Primary `wb_funnel_daily`** (WB Analytics API `sales-funnel`, ВКЛЮЧАЕТ
>    рассрочку, parity с Воронкой ЛК). Работает только для последних **7 дней**
>    (WB API rolling-окно, см. `WB_API_REFERENCE.md` § sales-funnel). Daily
>    sync 06:00 MSK накапливает локальную историю.
>
> 2. **Retro-fallback (>7 дней назад):**
>    - **Заказы** = `wb_orders ∪ wb_report_detail` по `srid` (union ловит
>      рассрочка-заказы, которые дошли до отчёта реализации). НЕ покрывает
>      отмены до доставки — их нет ни там, ни там. Это документированный
>      gap; для 100% паритета с Воронкой нужен extension (TASK-LEAD-158).
>    - **Выкупы** = `wb_report_detail` (Σ qty для Продажа − Σ qty для
>      Возврат) — АВТОРИТЕТНЫЙ финансовый отчёт, сходится с ЛК до штуки,
>      включает рассрочку (если реализовалась).
>
> 3. **Last-resort `wb_orders`/`wb_sales`** — для свежих nm без funnel и без
>    report_detail (новинки, ещё не закрытые отчёты).
>
> Деньги/аналитика рентабельности от рассрочечного разрыва не страдают (всегда
> из отчёта реализации). Через ~3 месяца после включения funnel-sync периоды
> BD/BE (полгода-год назад) тоже начнут покрываться.
