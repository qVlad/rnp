...
> ⚠️ **Источники по периоду (TASK-LEAD-153):**
>
> - **Период ≤ 7 дней назад от сегодня** — primary `wb_funnel_daily` (WB
>   Analytics API `sales-funnel`, ВКЛЮЧАЕТ рассрочку, parity с Воронкой ЛК).
>   Бакетируется ежедневным sync 06:00 MSK.
> - **Период > 7 дней назад** — fallback `wb_orders`/`wb_sales` (Statistics
>   API, без рассрочки, недосчёт ~16-20%). WB Analytics API имеет жёсткое
>   rolling-окно 7 дней (см. `WB_API_REFERENCE.md` § sales-funnel) — backfill
>   истории через API невозможен. Глубина funnel-данных накапливается
>   локально начиная с момента включения (TASK-LEAD-153, 2026-05-28).
>
> Деньги/аналитика рентабельности от рассрочечного разрыва не страдают (берётся
