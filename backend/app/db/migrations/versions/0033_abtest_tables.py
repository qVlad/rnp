...
def upgrade() -> None:
    # ------------------------------------------------------------------
    # Idempotency: на проде до этого merge abtest-таблицы были созданы
    # старой "0031" миграцией (потом переименовали в этот файл). Если
    # таблица `abtest` уже существует — пропускаем, считаем applied.
    # Это делает миграцию безопасной при post-merge recovery (stamp 0030
    # → upgrade head — без падения «relation already exists»).
    # ------------------------------------------------------------------
    bind = op.get_bind()
    inspector = sa.inspect(bind)
    if "abtest" in inspector.get_table_names():
        return
