...
from app.db.models import WbRedeemNotification, WbReportDetail
    ...
    *,
    ...
    Параметры:
        ...
        pay_offset_days — сколько дней прибавлять к report_date_to чтобы
            получить proxy для даты зачисления денег WB → р/с.
            Default 10 — соответствует данным со страницы «История платежей»
            ЛК WB: типичный лаг 7-11 дней между формированием заявки на
            оплату (понедельник после отчётной недели) и фактическим
            зачислением. Аномалии (20+ дн) бывают на праздниках.
            Публичного API для реальных pay-date нет (см. wb-api-specialist
            research, май 2026), поэтому если нужна точность 1:1 с банком
            — придётся делать XLSX-импорт «Истории платежей» из ЛК.
    """
    ...
    for r in realizations:
        ...
        pay_date = rdt + timedelta(days=pay_offset_days)
        bank_month = _month_key(pay_date)
        vzz_month = _month_key(rdt)
        sale_net = r["sale_net"]
        ppvz_net = r["ppvz_net"]
        vzz = max(0.0, sale_net - ppvz_net)  # удержания WB ≥ 0

        # Bank — по pay_date_proxy
        if bank_month in monthly:
            monthly[bank_month].bank += ppvz_net
            monthly[bank_month].realizations_count += 1
            monthly[bank_month].buyback_returns += r["returns_ppvz"]
    ...
    return {
        ...
        "methodology": (
            ...
            "Налог = База × ставка АУСН. Pay-date proxy = "
            f"report_date_to + {pay_offset_days} дн. ВЗЗ = "
            "retail_amount_net − ppvz_for_pay_net (удержания WB через "
