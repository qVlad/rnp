...
## 2026-07-05 — **TS-графики дашборда: «Период в графике», Топ-5 маржинальных, Структура выручки** (TASK-DEV-097, v0.91.0)

По скриншоту пользователя — три TS-блока главной: мультиметричный график по
дням (8 метрик, чекбоксы, prev-пунктир, 📌-маркеры; `/api/dashboard/period-chart`,
заменил «Динамику выручки»), pie топ-5 по марже (/top-skus), бары структуры
выручки (totals /extended-kpis). Director/head.

## 2026-07-04 — **TS-паритет пакет 2: /truestats, недельный свод, постатейный OPEX, аналитика РК** (TASK-DEV-096, v0.90.0)

Независимая живая сверка всех 17 разделов TS → закрыты последние гэпы:
- **/truestats** — UX-профиль меню «TrueStats» (только TS-эквивалентные разделы).
- **Сводный «По неделям»** — строки-ISO-недели, ~40 колонок, Redis-кэш
  закрытых недель (`/api/summary-report/weekly`).
- **Постатейный OPEX в P&L** — раскрытие по категориям (`/api/pnl/opex-breakdown`).
- **Аналитика РК v2** — зоны показа, воронка корзины, CPM/CPO/CPL/CPS, цены
  СПП/остатки per-кампания, drill по дням.
- Карточка товара: размеры + склады; окно прогноза РНП 7/28; **0090 cogs.vat_rub**
  + импорт XLSX на /cost-history.
- Попутно: BUG-DEV-030 (rnp/matrix 500 — Row.t коллизия SQLAlchemy) исправлен
  hotfix'ом v0.89.2 ещё до пакета.
- НЕ переносим: TS /tariffs + реферальный кабинет (их биллинг); ассоц. заказы
  (нет в публичном WB API).
...
старого аудита (2026-06-13) оказались уже сделаны — реальный список собран
заново с прода TS.
- **Движок `services/summary_metrics.py`** (вынос summary_report): 37 плиток
  (`SummaryTiles`, реюз /summary-report + Dashboard collapsed), «Исходная
  таблица» ~55 колонок (`SummaryTable`+`ColumnSettingsDrawer`), group_by=imt,
  include_prev, ABC×2, XLSX-экспорт, `GET /api/dashboard/extended-kpis`.
- **Источники, сверенные с живым TS (Onyx, закрытая неделя 22-28.06):**
  реализация/продажи/факт-комиссия/эквайринг/цены/штрафы — копейка-в-копейку;
  «Заказы» = ВОРОНКА (ordersCount, вкл. рассрочку; Δ 0.3% лаг синка), НЕ Лента;
  номинальная комиссия = `paid_storage_kgvp` + ЗНАКОВАЯ поправка из
  unit_plan_global_config (34.5 + (−0.75) = 33.75%); GMROI = прибыль/
  капитализация×100 (формула совпала с TS); % выкупа — терминальный → каскад
  fallback (cancel_count копится с DEV-087). Итоговое вознаграждение ВБ —
  остаток Δ~4% (документная метрика, TASK-DEV-061). Капитализация — снапшоты
  разные с TS by-design.
- **Комментарии-треды** (0086): 💬 на KPI/SKU/складах/строках РНП/планах,
  батч-счётчики `/api/comments/counts`.
- **РНП-матрица** (`services/rnp_matrix.py`): метрики×дни, типы кампаний
  (6=Поиск, 4/5/7/8=Полки+Каталог, 9=Единая), прогнозная прибыль с OPEX;
  `/rnp-settings` (0087). Ассоциированные заказы — в WB API НЕТ (строки нет).
- **План-факт**: breakdown день/неделя/месяц + прогресс-карточки.
- **Склады**: капитализация per склад; «Соответствие товаров» (0088);
  **/files** (UNION журналов); **email-приём выписок** — beat
  `tasks_email.poll_email_statements` каждые 30 мин (IMAP, настройки
  finance_email_* per-tenant, пароль Fernet, настройка на /finance-extras).
- Прод: миграции 0086-0088, pytest 27 новых+смежных / 299 passed (10 failed
  pre-existing), API-смоук всех эндпоинтов, смоук-коммент удалён.
