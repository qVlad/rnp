...
| **Transit calculator** (TASK-LEAD-077, 2026-05-22) | Страница `/transit-calculator` — расчёт стоимости **транзитной поставки** WB (хаб → конечный склад). WB Tariffs API НЕ отдаёт транзитные тарифы (доступны только в ЛК WB → Поставки → Транзитные направления), поэтому тариф ₽/л вводится вручную. Input: хаб (свободный текст + datalist с 12 известными хабами), конечный склад (dropdown из `wb_tariff_box`), количество шт, литров/шт, дней хранения, тариф small/large ₽/л с настраиваемым порогом (default 1500 л — двухступенчатая шкала WB). Output: транзит (`total_volume × rate`) + хранение на конечном складе (по обычному тарифу `wb_tariff_box`) + total + **compare с прямой поставкой** (Δ ₽, цветной — warn если транзит дороже). Frontend-only, persist в `localStorage["transit-calculator.v2"]`. Формула проверена против инструкции WB и сторонних руководств (research: `agents/references/research-transit-shipments-2026-05-22.md`). Помечена «рабочая гипотеза» — собственник корректирует тарифы по фактическим выгрузкам из ЛК. | `pages/TransitCalculator.tsx` (новый, 2026-05-22), `api.tariffWarehouses()` + `api.tariffCurrent("box")`, меню «SKU и продажи» → «Калькулятор транзита» | brands-filter |
