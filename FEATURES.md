...
| **Plan edit requests** (TASK-DEV-017) | Manager на `/plans` жмёт «✎ Предложить правку» → модалка (поле + значение + комментарий) → POST `/api/plan-edit-requests` + TG-notify. Director видит inbox-секцию сверху с pending-заявками + accept/reject. Accept применяет значение + audit_log на `sales_plans.update`. Reject — обязательный note. Whitelist полей planned_*. | миграция 0053, model `PlanEditRequest`, `api/plan_edit_requests.py`, `pages/Plans.tsx` | manager → director_or_head |
