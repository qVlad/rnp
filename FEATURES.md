...
| **Transit calculator** (TASK-LEAD-053) | Страница `/transit-calculator` — расчёт стоимости транзитной поставки на конкретный склад WB. Input: склад (dropdown из `wb_tariff_box`), количество шт, литров на штуку (`ceil`-логика WB), длительность хранения в днях. Output: per-unit breakdown (база + доп. литры × коэффициент склада + хранение) + total. Тарифы из таблицы `wb_tariff_box` (миграция 0040, SCD2, sync 08:00 MSK ежедневно). Persist параметров в `localStorage`. Сноска «что НЕ входит» (внешняя логистика, acceptance_fee, COGS, WB-комиссия) — честно очерчены границы. | `pages/TransitCalculator.tsx`, использует `api.tariffWarehouses()` + `api.tariffCurrent("box")` из `api/tariffs.py`, меню «SKU и продажи» → «Калькулятор поставки» | brands-filter |
