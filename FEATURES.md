...
| **Глобальные фильтры (DEV-062)** | Панель `<GlobalFilterBar>` (бренды/категории/группы/артикулы, мультиселект) на Сводном отчёте / Прочих удержаниях / Units / **Dashboard** / **P&L (table/cards/by-brand)** / **ABC** / **Аналитика РК** / **Тепловая карта рекламы (/ads)**. `FilterContext` (localStorage `globalFilters.v1`) → query-params; backend `services/filter_scope.py:resolve_nm_scope` сводит комбинацию (INTERSECT измерений ∩ RBAC) к `set[nm_id]`, который протянут через `metrics.py` (`nm_ids` во всех aggregate-fn + `compute_dashboard`/`top_skus`/`revenue_timeseries`), `pnl_builder.build_pnl` (→ contribution-margin), `abc_xyz.build_abc_xyz`, `kpi_breakdown`. Опции — `GET /api/filters/options` (каскад). **Магазины (Phase C, мульти-кабинет):** выбор ≥2 кабинетов → «свод» (сумма по выбранным tenant'ам). Механизм — `tenant_context.set_tenant_filter`: ORM-listener расширяет фильтр на `tenant_id IN (...)` (валидация по `user_tenant_access`), primary tenant сохраняется для AppSetting/writes (pitfall #16). P&L в своде → contribution-margin (OPEX/налоги per-tenant не сводятся). Дропдаун «Магазины» виден если у user >1 кабинета. | `contexts/FilterContext.tsx`, `components/GlobalFilterBar.tsx`, `api/filters.py`, `services/filter_scope.py` (`resolve_nm_scope`/`resolve_store_scope`), `services/tenant_context.py` | brands-filter (intersect; bookkeeper — без бара) |
...
| **Режим отчётности operational/financial** (TASK-LEAD-054) | Глобальный toggle в Layout-footer, ортогональный к `mode=preliminary/final/hybrid`. Группирует `wb_report_detail`: **operational** (default) = по `sale_dt` (день выкупа, как на дашборде WB-кабинета); **financial** = по `rr_dt` (день платёжки, как в WB-«Финансы → Реализация», для сверки с банком и УПД). Влияет на final-источник; preliminary остаётся без изменений (нет аналога rr_dt). Передаётся в `/api/dashboard*`, `/api/pnl*` через query-param `reporting_mode=operational\|financial`. Persist в `localStorage["reportingMode.v1"]`, cross-tab sync через `storage` event. | `contexts/ReportingModeContext.tsx`, `components/Layout.tsx`, `services/period_aggregates.py:get_period_filter`, `services/metrics.py`, `services/pnl_builder.py`, `api/dashboard.py`, `api/dashboard_compare.py`, `api/pnl.py` | all (кроме bookkeeper) |
| **Еженедельный отчёт менеджера** (TASK-LEAD-051) | Страница `/weekly-report` — компактный digest для понедельника-утра по выбранной неделе (default — последняя закрытая, навигация ←/→/⏎). Включает: 10 KPI grid с WoW дельтами (good-direction-aware: рост выручки = зелёный, рост returns = красный), Top-5 SKU по выручке + Top-5 по марже, активные alerts из AlertsBar, ручной комментарий менеджера (пока в localStorage — см. TASK-LEAD-062 для серверного хранения), PDF-экспорт через `html2pdf.js` для отправки РОПу. Backend не отдельный — переиспользует `/api/dashboard` (`mode=final`), `/api/dashboard/top-skus`, `/api/dashboard/alerts`. Manager видит свой brand-scope, head/director — без фильтра. | `pages/WeeklyReport.tsx`, меню «Отчёты» → «Еженедельный отчёт» | brands-filter |
| **Multi-manager scoreboard в `/weekly-report`** (TASK-LEAD-061) | Над KPI-сеткой `/weekly-report` (видна только для `director` / `head_of_sales`, manager не видит): таблица «По менеджерам» со всеми активными менеджерами тенанта. Колонки: Менеджер · Бренды · Выручка · Маржа (₽ + %) · WoW выручки · WoW маржи (п.п.) · Заказов · Возвратов. Сортировка кликом по любому заголовку (default — выручка desc). Менеджеры без `brand_assignments` всегда внизу (с пометкой «не назначены»). Empty state — ссылка на `/brands`. Backend: новый `GET /api/weekly-report/by-manager?week_start=YYYY-MM-DD` (`require_director_or_head`) → группировка через `brand_assignments`, агрегация per-manager через `services/metrics.compute_dashboard(mode=final)` × брендовый set, WoW считается относительно `week_start - 7 дней`. WoW дельты — через существующий `DeltaCell` (good-direction-aware). | `pages/WeeklyReport.tsx` (секция «По менеджерам»), `api/weekly_report.py`, `services/weekly_report.py` | director / head_of_sales |
| **Top-3 рекомендации в `/weekly-report`** (TASK-LEAD-064) | Секция «Top-N действий на эту неделю» в самом верху страницы (между Header и Scoreboard / KPI grid). Эвристики на уже синхронизированных данных: **stockout** (`wb_stocks_snapshot.quantity_full == 0` И `orders_30d > 5` из `wb_sales`) → «#X закончился — нужна поставка», severity=high; **drr_high** (`ad_spent ≥ 1000₽` И `DRR > 20%` за неделю по `wb_report_detail` × `wb_ad_stats_daily`) → «#X DRR Y% — снизить ставки», severity=high; **returns_high** (`returns/orders > 30%`, минимум 5 заказов) → «#X возвраты Y% — проверить размерную сетку», severity=medium. Сортировка severity desc → revenue_impact desc; топ-3. Каждый item кликается на `/units?nm_id=X`. Скрыта если рекомендаций нет. Backend: `services/weekly_recommendations.py` + `GET /api/weekly-report/recommendations?week_start=YYYY-MM-DD` (brands-filter применяется автоматически — manager видит свой scope). | `pages/WeeklyReport.tsx`, `api/weekly_report.py`, `services/weekly_recommendations.py` | brands-filter (исключая bookkeeper) |
| **Send-to-Telegram из `/weekly-report`** (HYP-002) | Кнопка «📨 Отправить в Telegram» рядом с «Скачать PDF». При клике — preview-запрос со списком потенциальных recipient'ов, confirm-диалог («Отправить отчёт N директорам: Иванов, Петров?»), затем HTML-сообщение через `services/tg_broadcast.broadcast_to_directors`. Содержание: header (бренд / период), Top-5 KPI с WoW (выручка / маржа / orders / ДРР), Top-3 рекомендации (если есть), комментарий менеджера (`weekly_report_comment.brand=null`, обрезанный до 500 симв.), ссылка обратно. RBAC: director/head → broadcast всем директорам (recipient_filter=`all_directors`); manager → принудительно self-чат (`users.tg_chat_id` собственный). Если у юзера/команды нет привязки TG → `{shared: false, fallback: "download_pdf"}` → автоматический PDF-export как fallback + toast «Нет привязки TG, скачали PDF — отправь вручную». | `pages/WeeklyReport.tsx`, `api/weekly_report.py`, `services/tg_broadcast.py` | all (manager → self; director/head → broadcast) |
...
| **Inline-редактор цены и скидки** (TASK-LEAD-049) | На странице `/units` колонки «Новая цена» и «Новая скидка» редактируются прямо в таблице. После изменения автоматически пересчитывается «Новая маржа» с цветной дельтой (▲/▼) к текущей. Помогает РОПу/менеджеру понять «что будет с маржой если я подниму цену на 50₽» без выгрузки в Excel. Per-nm subscription store (`useSyncExternalStore`) — keystroke в одной строке не дёргает остальные 49 строк (perf-фикс BUG-DEV-007 follow-up #2). Изменения локальные (preview), сохранение через WB-кабинет вручную. | `pages/Units.tsx` (cells `NewPriceCell` / `NewDiscountCell` / `NewMarginCell`), меню «SKU и продажи» → «Юнит-экономика» | brands-filter |
...
| Фича | Описание | Путь в коде | Доступ |
|---|---|---|---|
| **TS-графики дашборда (DEV-097)** | Три TS-блока на главной: (1) **«Период в графике»** — мультиметричный график по дням глобального периода (Продажи / Ср.цена до скидок МП / Заказы ₽ / ДРРп % / Логистика / Возвраты / Чистая прибыль / Хранение; чекбоксы с persist, двойная ось ₽/%, «Сравнить с прошлым периодом» — пунктир prev, наложенный по индексу дня, «Комментарии» — 📌-маркеры аннотаций). `GET /api/dashboard/period-chart` (director/head): report_detail через period_aggregates + wb_orders + ad_stats + build_pnl(day) для прибыли; заменил блок «Динамика выручки» (панель заметок сохранена); (2) **«Топ 5 маржинальных товаров»** — pie долей маржи топ-5 + «Остальные» (fронт из /top-skus?by=margin&limit=50); (3) **«Структура выручки (статьи)»** — горизонтальные бары (Скидка МП/Логистика/Себестоимость/Реклама/OPEX/Прочие удержания/Налоги/Хранение/Прибыль/Комиссия/Прочее) с суммами, из totals /extended-kpis | `api/dashboard.py:get_period_chart`, `components/dashboard/{PeriodChart,TopMarginPie,RevenueStructureBars}.tsx` | director, head |
| **TS-паритет пакет 2 (DEV-096)** | Независимая сверка с живым TS (2026-07-04, все 17 разделов) → закрытие последних гэпов: (1) **сводный отчёт «По неделям»** — строки-ISO-недели + «Итого за период», колонки = totals summary-движка, Redis-кэш закрытых недель (`/api/summary-report/weekly`, вкладка на /summary-report); (2) **постатейный OPEX в P&L** — раскрытие строки «OPEX» по категориям opex_entries × бакеты (`/api/pnl/opex-breakdown`); (3) **расширенная Аналитика РК** — вкладки «По зонам показа» (Поиск/Полки+Каталог/Единая) и «По кампаниям» (~20 колонок: воронка корзины atb%/order%, CPM/CPO/CPL/CPS, цены до/после СПП и остатки по nm-составу кампании, настройка колонок), drill «По дням» (`/api/ad-campaigns/analytics` + `/daily`); (4) **карточка товара**: «Аналитика по размерам» (реюз units/{nm}/sizes) + «Наличие на складах»; (5) **окно усреднения прогноза РНП** 7/28/период (`/api/rnp/forecast-window`, AppSetting per-tenant, «Настройки расчёта» на /rnp-settings); (6) **cogs.vat_rub** (0090) + импорт/экспорт XLSX прямо на /cost-history; (7) **UX-профиль меню «TrueStats»** + роут `/truestats` (включает профиль → дашборд). Найден и исправлен BUG-DEV-030 (rnp/matrix 500). Не переносим: TS /tariffs и реферальный кабинет (их биллинг), ассоц. заказы (нет в публичном WB API) | `api/finance_extra.py`, `api/pnl.py`, `api/rnp.py`, `pages/AdCampaignsAnalytics.tsx`, `components/WeeklySummaryTable.tsx`, миграция 0090 | director, head |
| **Ревизии WB-отчётов (DEV-095)** | Периодическая переподгрузка исторических WB-отчётов со сравнением: свежие строки диффятся с БД (`services/wb_revision.diff_and_apply`, допуск 0.01), применяются в основные таблицы (актуальность), а прежние значения изменённых строк — в журнал `wb_sync_change` (added/updated/**rejected_lower**). FREEZE (Правило 3.5): понижение защищённого поля (реклама `sum_spent`) НЕ применяется, но журналируется — расхождение видно, история не портится. Источники: report_detail (finance-api, любой период, weekly сб 01:45, 42д), ad_stats (fullstats ≤31д-чанки, daily 02:20, 30д), orders/sales (weekly вс 01:25/05:25, 45д), funnel (rolling-7 ЖЁСТКО — только внутри окна, daily 06:50). UI `/data-revisions` («Контроль → Ревизии WB»): список ревизий со счётчиками и Δ сумм, просмотр изменений (было/стало, paged), ручной запуск per-source с лимитами глубины. Маппинг-функции синков вынесены в реюз: `_map_order_row/_map_sale_row/_map_report_detail_row/_fullstats_values` | `services/wb_revision.py`, `sync/tasks_refetch.py`, `api/data_revisions.py`, `pages/DataRevisions.tsx`, миграция 0089 | director, head |
| **TS-паритет пакет (DEV-094)** | Осмотр живого TS 2026-07-02 → закрытие оставшихся гэпов одним релизом: (1) **движок `services/summary_metrics.py`** (вынос summary_report): 37 KPI-плиток (`SummaryTiles`, реюз на /summary-report и Dashboard collapsed-секцией) — добавлены GMROI + годовой (прибыль/капитализация×100, аннуализация; остатки копятся с 05.2026), номинальная комиссия (WbTariffCommission по subject, SCD2 as-of), ДРР бонусов + Общая ДРР, own-склады из `off_platform.summary`; заказы в totals = wb_orders («Лента», как TS), order-only SKU включаются нулевыми строками; (2) **«Исходная таблица»** ~55 колонок per-SKU (`SummaryTable` + `ColumnSettingsDrawer`, persist localStorage; штрафы/приёмка/компенсации/вознаграждение ВБ/номинал per-SKU, заказы+%выкупа, цены, остатки ×4 + капитализации + оборачиваемость + GMROI per-SKU, прибыль ×2, доля выручки, ABC по прибыли и выручке, реклама ×5), `group_by=imt` (склейки), `include_prev` (дельты), XLSX-экспорт; (3) **РНП-матрица** метрики×дни (`services/rnp_matrix.py` + вкладки в `RnpModule.tsx`, «Настройка строк», типы кампаний Поиск/Полки+Каталог/Единая, прогнозная прибыль с OPEX по коэффициентам summary-движка) + `/rnp-settings` (0087); (4) **комментарии-треды** (0086); (5) **план-факт**: breakdown день/неделя/месяц + прогресс-карточки (`MetricPlanFact.tsx`); (6) капитализация per склад (`stocks/by-warehouse`) + «Соответствие товаров» (0088) + **`/files`** (единый журнал) + **email-приём выписок** (`sync/tasks_email.py`, beat 30 мин, IMAP, пароль Fernet, настройки на /finance-extras). Маркер «Сегодня» в календаре операций; KPI «Итого к оплате». Ассоциированные заказы TS — в публичном WB API нет (строка не показывается, честно) | `services/summary_metrics.py`, `components/SummaryTiles.tsx`, `SummaryTable.tsx`, `CommentThread.tsx`, `services/rnp_matrix.py`, `api/files.py`, `sync/tasks_email.py` | director, head |
| **ДДС-матрица (DEV-093, TS-стиль)** | `/cash-flow` перестроен: матрица «статьи × месяцы» из банковских/ручных операций (сумма + % в ячейке, раскрытие Доход/Расход, строки Нераспределённый/Перевод/Сальдо/накопленный остаток), вкладки По статьям / По виду деятельности / По контрагентам / **Управленческий (WB+OPEX)** (старый рендер, сохранён). Хедер: «Остаток на счетах» с per-счёт дропдауном, ссылки «N операций без статей» (→ /operations?no_article=1) и «N ошибок импорта», чекбокс «Учитывать плановые». `GET /api/cash-flow/matrix?group_by=article\|activity\|counterparty` | `pages/CashFlow.tsx`, `services/cash_flow_matrix.py`, `api/finance_ops.py` | director, head |
| **Счета (DEV-093)** | `finance_account` (миграция 0083): начальный баланс (+дата), архив; текущий баланс ВЫЧИСЛЯЕТСЯ (initial + Σфакт-операций, переводы двигают оба счёта). CRUD `/api/finance-accounts` (delete → 409 при операциях, только архив). `bank_meta` JSONB — задел под Т-Банк API | `services/finance_accounts.py`, `pages/FinanceExtras.tsx` (вкладка Счета) | director, head |
| **Операции (DEV-093)** | `/operations` → 3 вкладки: **Операции** (лента всех source: фильтры счёт/статья/тип/источник/официальный/без-статьи/поиск, inline-выбор статьи кликом по «Операция без статьи», бейджи «✓ Официальный расход»/«⚡ автоправило»/источник, форма с **Переводом между счетами**, виды Таблица/**Календарь** — месячная сетка с балансом дня), **Импорты** (журнал выписок), **WB-операции** (report_detail, как раньше). PUT `/api/manual-operations/{id}`, PATCH `/bulk`, GET `/export.xlsx`. Totals исключают transfer | `pages/Operations.tsx`, `components/finance/OperationsCalendar.tsx`, `api/finance_extra.py` | director, head |
| **Импорт банковской выписки (DEV-093)** | 1С «1CClientBankExchange» (.txt, декод-каскад utf-8→cp1251, направление по своему р/с) + Excel/CSV (наш шаблон или произвольные колонки → мастер маппинга). Превью → выбор счёта → commit: операции source='import', **дедуп** sha256(счёт+дата+сумма+№дока+назначение) — повторная загрузка не задваивает; прогон автоправил. Журнал `finance_import_batch` (0084), удаление ± операции. `POST /api/finance-imports` (+`/commit`), `GET /template.xlsx` | `services/bank_statement.py`, `components/finance/ImportWizard.tsx` | director, head |
| **Автоправила (DEV-093)** | Условия (контрагент/назначение/сумма/тип: equals/contains/gte/lte, AND) → действия (статья/контрагент — только в ПУСТЫЕ поля, официальный расход — всегда). Прогон при импорте (первое по priority) + «⚡ применить к существующим». `finance_auto_rule` (0085), CRUD `/api/finance-rules` | `services/finance_rules.py`, `pages/FinanceExtras.tsx` (вкладка Автоправила) | director, head |
| **Статьи с типом/видом деятельности (DEV-093)** | FinanceReference(expense_category) + `extra.op_type` (Доход/Расход/Перевод) + `extra.activity` (Операционная/Инвестиционная/Финансовая) — редактируются на вкладке Статьи; кнопка «Импортировать из OPEX» (`POST /api/finance-reference/import-opex`, kind→op_type, cf_section→activity, идемпотентно). PUT `/api/finance-reference/{id}` | `pages/FinanceExtras.tsx` | director, head |
| **Плановые из WB (DEV-093)** | Настройки (вкладка на /finance-extras): toggle «Создавать плановые операции из отчётов маркетплейсов» → `POST /api/finance-plan/sync-wb-payouts` создаёт/обновляет операции source='auto_plan', is_planned=true из ожидаемых выплат WB (report_date_to+lag, идемпотентно по doc_number `wb-<realization>`; оплаченные гасятся). AppSetting-ключи `finance_auto_confirm_planned`/`finance_auto_plan_wb_payouts` (tenant-filtered, pitfall #16) | `api/finance_ops.py` | director, head |
| Cash flow (ДДС управленческий) | Движение денежных средств из WB (P&L) + OPEX с разделением operating / investing / financing — вкладка «Управленческий (WB+OPEX)» на /cash-flow. `GET /api/cash-flow` не менялся | `pages/CashFlow.tsx`, `api/cash_flow.py`, миграция 0005 | director, head |
| Payment calendar | Прогноз WB payouts + scheduled OPEX → daily balance curve. Initial balance по умолчанию = Σ балансов finance_account (DEV-093; fallback — AppSetting `bank_balance_current`, теперь с tenant-фильтром — был кросс-tenant баг) | `pages/PaymentCalendar.tsx`, `services/payment_calendar.py` | director, head |
...
| **Кабинеты WB — управление (DEV-092)** | Один сервис-аккаунт → N кабинетов WB (по образцу TrueStats). `/settings` → секция «Кабинеты WB» (`CabinetsSection.tsx`): таблица (название/seller_id/статус токена/роль/бейдж «активный») + модалка «Добавить кабинет» (название + токен, «Проверить» через tenant-agnostic `/api/tenant/wb-token/validate`, 409 `duplicate_seller` + force). Router `api/tenants.py`: `GET/POST /api/tenants`, `PATCH /{tid}` (rename/hidden), `PUT/DELETE /{tid}/wb-token`, `GET/POST/DELETE /{tid}/access`. POST: ping ДО создания tenant'а → слаг → **репликация доступов всем user'ам текущего кабинета с их ролями** → токен (Fernet) → audit в новый tenant → авто-sync 90д (`services/wb_token.trigger_initial_sync`). Удаления НЕТ: «Отключить токен» (sync стоп, данные живут) + «Скрыть» (`tenants.hidden_at`, миграция 0082) — архив, исключается из available-tenants/свода/sync-диспетчеров, «Вернуть» отменяет. Скрыть последний видимый кабинет нельзя. Guard: director именно этого кабинета (`_require_cabinet_director` по `user_tenant_access`), запрет отзыва последнего director'а. | `api/tenants.py`, `services/wb_token.py`, `components/CabinetsSection.tsx`, миграция 0082 | director (per-cabinet) |
| **Свод по умолчанию (DEV-092)** | При ≥2 видимых кабинетах director/head видит на аналитических страницах **сумму по всем кабинетам без выбора** (как TrueStats); фильтр «Магазины» сужает (1 выбранный = данные только его). `resolve_store_scope`: `stores` не передан → все не-hidden кабинеты user'а. SKU-уровень — через `set_tenant_filter` (tenant IN); **финансовые агрегаты (P&L, Dashboard net_profit) — `build_pnl_consolidated`**: цикл по кабинетам (`set_tenant` → полный P&L со СВОИМИ AppSetting-налогами/OPEX, pitfall #16, заодно нет коллапса COGS BUG-DEV-025) → суммирование raw-полей + пересчёт derived/`*_pct` (`_recompute_derived`). Ответы получают `consolidated: N`; UI-бейдж «Свод: N каб.» в GlobalFilterBar. P&L свода = полный (scope=company), НЕ contribution-margin. Manager/bookkeeper — без свода (один активный кабинет). | `services/filter_scope.py:resolve_store_scope`, `services/pnl_builder.py:build_pnl_consolidated`, `services/metrics.py:compute_dashboard(store_ids)`, `api/pnl.py`, `api/dashboard.py`, `components/GlobalFilterBar.tsx` | director, head |
| Users CRUD | Управление пользователями (4 роли). `create_user` создаёт `UserTenantAccess` (BUG-DEV-029), смена роли синхронизирует per-tenant роль | `pages/Users.tsx`, `api/users.py`, миграция 0012 | director |
...
| Фича | Описание | Путь в коде | Доступ |
|---|---|---|---|
| WB token CRUD | Установка/удаление токена per-tenant с шифрованием Fernet. Работает с **активным** кабинетом (BUG-DEV-028), общая логика — `services/wb_token.py` (реюз в `api/tenants.py`) | `api/tenant_settings.py`, `services/wb_token.py`, `services/secrets_crypto.py` | director |
| WB token validation | Ping `/ping` перед сохранением. Tenant-agnostic — используется и модалкой «Добавить кабинет» | `services/wb_token.py:ping_wb` | director |
| **Auto-trigger sync** | При первой установке токена — автоматически запускается 90-дневный backfill всех 8 сущностей | `services/wb_token.py:trigger_initial_sync` | — |
| Seller ID extraction | Распарка JWT WB-токена → `wb_token_seller_id` | `services/wb_token.py:decode_wb_token_sid` | — |
...
| **Promo calculator** (TASK-LEAD-050) | Калькулятор рентабельности WB-акций: пользователь выбирает SKU (multi-select), параметры акции (скидка %, длительность, ожидаемый velocity boost), baseline-период (7/14/30 дней). Backend `simulate_promo_for_skus` берёт baseline из `wb_report_detail` (revenue/velocity/margin/commission/logistics per SKU), считает with-promo сценарий (new_price = price × (1−discount), new_velocity = velocity × (1+boost), new_margin = new_price − cogs − comm − log). Возвращает per-SKU baseline vs with-promo + delta + breakeven velocity boost (минимальный boost для безубытка). Color-coding в таблице: зелёный фон = better than baseline, красный = убыток per unit. Сортировка по delta_margin desc. WB Promo Calendar API (`dp-calendar-api.wildberries.ru`) пока используется только как опциональный source для preload активных акций (`integrations/wb/promotions.py`) — graceful fallback на manual-input при недоступности. Методичка: [`PROMO_CALCULATOR.md`](PROMO_CALCULATOR.md). **TASK-DEV-030 (2026-06-02):** на `/promo-calculator-wb` добавлен режим «Сравнение акций» — матрица per-unit маржи: строки = union SKU выбранных акций (до 6), столбцы-группы = «Текущие продажи» + каждая акция × (цена без СПП / маржа ₽ / маржа %). Цена акции = WB discountedPrice по SKU (или baseline×(1−override%) при ручном override). `POST /api/promo-calculator/compare` → `compare_promos_for_skus` + `_margin_at_price` (reuse `_load_baselines`). SKU без baseline-продаж пропускаются. **TASK-DEV-037 ph4 (2026-06-03):** третий режим «📅 Лента акций» (default) — WB-подобный Gantt-таймлайн: акции горизонтальными полосами по датам (`start_dt`/`end_dt` из `wb_promotion`), месячная шапка, линия «сегодня», цвет = тип (accent обычная / amber авто / soft завершённая), ✓ = `in_promo_action`. Клик по полосе → `setMode('simulate')` + выбор акции. Компонент `PromoCalendar` в `PromoCalculatorWb.tsx`. **TASK-DEV-037 ph4.1:** переделан под оригинал WB — дневной таймлайн (день=116px, дорожки greedy-scheduling), фильтр-табы Доступные/Участвую/Не участвую (счётчики из `in_promo_action`/`products_count`), навигация ‹ › + «Сегодня», статус-бейджи (Участвую/Буду участвовать/Пропускаю/Участвовал), бустинг-пилюля «N из M%» (из `ranging`), «+K» инструментов, hover-тултип (dark, статус+даты+бустинг+advantages-пилюли). Список-эндпоинт `_db_list_promotions` отдаёт `boost_current/boost_max` (из ranging+participationPercentage), `advantages`, `participation_pct`. | `services/promo_calculator.py`, `api/promo_calculator.py` (`POST /api/promo-calculator/simulate` + `/compare`), `integrations/wb/promotions.py`, `pages/PromoCalculator.tsx` + `pages/PromoCalculatorWb.tsx`, тест `tests/test_promo_calculator.py` | brands-filter (director/head/manager) |
| **Leak-report «найдено ₽»** (TASK-LEAD-140, 2026-05-26) | Страница `/leak-report` — аудит-артефакт кабинета: одно число «найдено N₽» за период + breakdown по 5 источникам утечек, разделённым на **«вернуть»** (💰 recover) и **«прекратить»** (🩹 prevent). Источники: (1) оспоримые штрафы/чарджбэки — `Chargeback` со статусом `new`/`disputing`, кроме income-категорий, `amount_rub − recovered_amount`; (2) SKU проданные в убыток — `net_profit < 0` из `build_unit_economics`; (3) дохлый сток в платном хранении — `storage > 0 & stock > 0 & units_sold_gross ≤ 0`; (4) переплата логистики из-за перемеров WB — `(volume_l − prev_volume_l) × delivery_liter × units` (`WbProductDimensionsHistory × WbTariffBox`); (5) убыточные акции постфактум — промо-строки `wb_report_detail` (`seller_promo_id`/`seller_promo*`) где `Σppvz − COGS×units − Σdelivery < 0`. **Recon — не сумма, а trust-badge** «✅ сверено с WB: N/M недель совпали до 1%» (reuse `build_reconciliation`). Печатный/PDF-вид: кнопка «Печать / PDF» (`window.print()`), scoped print-CSS прячет `<aside>` сайдбар и контролы, в печати раскрываются все детали-таблицы. Назначение: ритуал входа в клуб (онбординг чужого кабинета) + sales-артефакт для предпродажи (memory `project-internal-tool` — коммерциализация через клуб-модель). | `services/leak_report.py`, `api/leak_report.py` (`GET /api/leak-report`), `pages/LeakReport.tsx`, меню «Дашборд и сверка» → «Найдено ₽ (аудит)» | director_or_head (brands-filter урезает manager-scope) |
| **Supply calculator** (TASK-LEAD-053, переименован TASK-LEAD-077 2026-05-22) | Страница `/supply-calculator` — расчёт стоимости **обычной (прямой) поставки** на конкретный склад WB. Input: склад (dropdown из `wb_tariff_box`), количество шт, литров на штуку (`ceil`-логика WB), длительность хранения в днях. Output: per-unit breakdown (база + доп. литры × коэффициент склада + хранение) + total. Тарифы из таблицы `wb_tariff_box` (миграция 0040, SCD2, sync 08:00 MSK ежедневно). Persist параметров в `localStorage["supply-calc.params.v1"]` с lazy-fallback на старый ключ `transit-calc.params.v1`. Сноска «что НЕ входит» (внешняя логистика, acceptance_fee, COGS, WB-комиссия) — честно очерчены границы. | `pages/SupplyCalculator.tsx` (был `TransitCalculator.tsx` до 2026-05-22), использует `api.tariffWarehouses()` + `api.tariffCurrent("box")` из `api/tariffs.py`, меню «SKU и продажи» → «Калькулятор поставки» | brands-filter |
| **Dimensions tracking** (TASK-LEAD-129, 2026-05-26) | Страница `/dimensions-history` + Celery beat `sync.product_volume` (05:45 MSK ежедневно). Тянет `dimensions: {length, width, height}` из WB Content API (`POST /content/v2/get/cards/list`) для всех карточек, сравнивает с `products.length_cm/width_cm/height_cm`. При diff → INSERT в `wb_product_dimensions_history` (миграция 0063, append-only лог с prev_* копиями для удобного UI-diff'а) + UPDATE products + TG-broadcast директорам через `services/tg_broadcast.broadcast_to_directors`. Первый замер пишется с `change_kind='initial'` без TG (initial snapshot). Селлер сразу видит «🔧 WB перемерил товар X (12345): 30×20×5 см → 32×22×6 см, объём 3.0 → 4.224 л (↑40.8%)» — раньше эту информацию находили только через странные цифры в `/unit-plan`. UI: таблица с фото, старые/новые габариты, Δ% объёма (вверх = красный, вниз = зелёный), toggle «только реальные перемерки» (скрывает initial). Manager видит только свои бренды (brands-filter). | `sync/tasks_product_volume.py`, `services/tg_broadcast.broadcast_to_directors`, `api/product_dimensions.py` (`GET /api/product-dimensions/history` + `/{nm_id}`, `POST /sync`), `integrations/wb/content.extract_dimensions`, `pages/DimensionsHistory.tsx`, миграция 0063 | GET — brands-filter (manager OK); POST /sync — director_or_head |
| **Transit calculator** (TASK-LEAD-077, 2026-05-22; auto-fetch TASK-LEAD-078, 2026-05-23) | Страница `/transit-calculator` — расчёт стоимости **транзитной поставки** WB (хаб → конечный склад). WB Tariffs API не отдаёт транзитные тарифы (доступны только в ЛК WB → Поставки → Транзитные направления). **TASK-LEAD-078:** Chrome-расширение перехватывает internal-fetch'и WB-фронта на странице «Транзитные направления», парсит таблицу `(hub → destination → rate)` по shape данных и постит на backend через `POST /api/transit-tariffs/upload`. **BUG-DEV-019 (2026-06-01):** реальный endpoint найден — `seller-supply.wildberries.ru/ns/sm/supply-manager/api/v1/plan/transitTariffsV2` (JSON-RPC, `{result:{items:[…]}}`); парсер подогнан под реальный shape: hub←`transitWarehouseName`, dest←`destinationWarehouseName`, двухступенчатый тариф ₽/л из `tariffTable.perVolume` (`[{from:0,to:1500,value},{from:1500,to:0,value}]`), плоский `currentTariff`/паллетные маршруты с пустым `perVolume` пропускаются. Страница ЛК — `/supplies-management/transit-report`. Миграция **0059** — `wb_transit_tariff(tenant_id, hub_name, destination_warehouse, rate_small, rate_large, threshold_l, currency, synced_at)` + UNIQUE `(tenant, hub, destination)`. UI auto-fill: при выборе пары hub+destination тариф подставляется автоматически с пометкой «📊 Из ЛК WB · обновлён N ч назад». **TASK-DEV-028 (2026-06-01):** выпадашки хаба и конечного склада строятся СТРОГО из реально подгруженных тарифов (`transitListQ` → distinct hub, склад фильтруется по выбранному хабу) — убран хардкод `KNOWN_HUBS`. Нет данных → баннер «🚫 транзит недоступен». Дата последней подгрузки (`max(synced_at)`) — чип в шапке; данные старше 3 дней → общий оранжевый stale-баннер (раньше per-pair 30д). Ручной ввод хаба/склада — за тогглом «✂ ввести вручную» (graceful fallback, страница по дизайну поддерживает manual-режим). Input: хаб (select реальных хабов / manual), конечный склад (select направлений из хаба / full `wb_tariff_box` в manual), кол-во шт, литров/шт, дней хранения, тариф small/large ₽/л с настраиваемым порогом (default 1500 л). Output: транзит + хранение на конечном складе + total + compare с прямой поставкой (Δ ₽). Persist в `localStorage["transit-calculator.v2"]`. Research: `research-transit-shipments-2026-05-22.md` (формула) + `research-transit-lk-endpoint-2026-05-22.md` (гибкий парсер shape). | `pages/TransitCalculator.tsx`, `api.transitTariffsList/Lookup()`, `api/transit_tariffs.py` (GET list/lookup, POST /upload), extension `wb-transit-tariffs-{interceptor-main,content}.ts` + SW `maybeUploadTransitTariffs`, меню «SKU и продажи» → «Калькулятор транзита» | GET — brands-filter (manager OK); POST /upload — director_or_head |
...
| **0048** | **extension_api_tokens** — long-lived токены `rnpext_<32-hex>` для Chrome-расширения (вместо 12h JWT в cookie). Бессрочный или TTL до 10 лет, revoke. UI в /settings → «Токены для Chrome-расширения». `api/extension.py:_user_from_bearer` поддерживает оба формата (rnpext_ → table lookup, else JWT). `POST/GET/DELETE /api/extension/api-tokens` |
| **0049** | **alert_acknowledgements** — серверный ack для AlertsBar (TASK-DEV-020). Заменяет `localStorage["alerts.dismissed.v2"]` на `(tenant_id, user_id, alert_code, signature, acknowledged_at)`, UNIQUE на `(tenant_id, signature)`. Один ack глушит алерт для всей команды; ФИО+время видны в «Прочитанные». Signature = sha1(`code\|message`)[:32] — message меняется → новый ack не наследуется. `POST /api/dashboard/alerts/ack` + `DELETE /api/dashboard/alerts/ack/{signature}` |
...
| **0051** | reconciliation_imports — журнал импортов в reconciliation |
| **0052** | **product_tags + product_tag_assignments** — эмодзи-теги на nm_id (TASK-DEV-024). 6 preset-тегов (🏆/⭐/📦/🆕/🚨/🔥) seed при создании tenant'а. M:N через UNIQUE (tenant, nm_id, tag_id). `/api/product-tags` CRUD (director) + `/api/products/{nm_id}/tags` GET/PUT (brand-scope) |
| **0053** | **plan_edit_requests** — заявки manager'а на правку плана (TASK-DEV-017). pending → accepted (=apply+audit) / rejected (=close с note). Whitelist полей planned_* (без metadata). TG-broadcast директорам (`services/tg_broadcast.py`). `/api/plan-edit-requests` POST+GET + `/{id}/accept` + `/{id}/reject` |
| **0054** | **users.tg_chat_id** — per-user Telegram binding для multi-recipient broadcast'а. `broadcast_to_directors` сначала шлёт всем `User.tg_chat_id IS NOT NULL` подходящей роли, fallback на legacy `AppSetting.tg_chat_id`. UI: /settings → «Мой Telegram-чат» |
...
| **0056** | **user_tenant_access** — M:N user↔tenant для multi-cabinet workspace (TASK-LEAD-048). Composite PK `(user_id, tenant_id)` + per-tenant `role` (в одной компании director, в другой manager). `last_active_at` для сортировки dropdown'а. Backfill: 1 запись на existing user'а из `users.tenant_id`+`role`. `users.tenant_id` — read-only legacy (drop в Фазу D). Middleware `services/active_tenant.py` резолвит active tenant (cookie `rnp_active_tenant` → header `X-Tenant-ID` → fallback). `GET /api/auth/available-tenants` + `POST /switch-tenant`, audit `tenant.switch` |
| **0057** | **wb_prices + wb_prices_size** — актуальные цены продавца из WB Prices API (TASK-LEAD-074). PK `(tenant_id, nm_id)` / `(tenant_id, nm_id, tech_size)`. Source of truth для базовой цены в /unit-plan (заменяет fallback на `wb_sales.price_with_disc`). Sync `sync.tasks_prices.sync_wb_prices` каждые 30 мин (full upsert). WB endpoint `GET /api/v2/list/goods/filter` на `discounts-prices-api.wildberries.ru` (6/min, min 10с). `_latest_price` возвращает `(price, discount_share, source, synced_at)`, source ∈ wb_prices/wb_sales/none → badge в UI. `GET /api/unit-plan/prices-status` + `POST /sync-prices` |
| **0058** | **weekly_report_comment** — серверный комментарий менеджера в /weekly-report (TASK-LEAD-062). Заменяет localStorage. `(tenant, brand, week_start, comment, author_user_id)`. `brand=NULL` = общий комментарий за неделю на весь tenant (РОП/собственник), `brand=…` = per-brand комментарий менеджера. UNIQUE через `COALESCE(brand, '__overall__')` |
| **0059** | **wb_transit_tariff** — тарифы транзитных направлений из ЛК WB (TASK-LEAD-078). WB Tariffs API публично транзит не отдаёт — данные поставляет Chrome-расширение (MAIN-world interceptor internal-fetch WB-фронта). `(tenant_id, hub_name, destination_warehouse, rate_small, rate_large, threshold_l=1500, currency, synced_at)` + UNIQUE `(tenant, hub, destination)`. `TransitCalculator.tsx` auto-fill. `GET /api/transit-tariffs` + `/lookup?hub&dest` (404 если нет) + `POST /upload` (director_or_head). Extension: `wb-transit-tariffs-interceptor-main.ts` (MAIN) + `-content.ts` (ISOLATED, FNV-1a дедуп), SW `maybeUploadTransitTariffs` |
| **0063** | **wb_product_dimensions_history + products.length/width/height_cm** — tracking перемерок WB (TASK-LEAD-129). WB меняет dimensions → объём растёт → логистика дороже → маржа падает. Append-only лог `(tenant_id, nm_id, length/width/height_cm, volume_l, prev_*, change_kind ∈ {initial, changed}, detected_at, source)` + INDEX `(tenant_id, nm_id, detected_at DESC)`. `products.length/width/height_cm` чтобы diff'ить без JOIN. `sync/tasks_product_volume.py`: тянет dimensions всех SKU, при diff (tolerance 0.01см) → INSERT history + UPDATE products + TG-broadcast «🔧 WB перемерил…». initial без TG. Beat 05:45 MSK. UI `/dimensions-history` (`pages/DimensionsHistory.tsx`) |
| **0068** | **wb_promotion + wb_promotion_nomenclature** — кэш WB-календаря акций (TASK-DEV-037 ph1-2). `wb_promotion` (PK id, uniq tenant+promotion_id; name/start_dt/end_dt/promo_type/counts/ranging JSONB/raw JSONB/synced_at), `wb_promotion_nomenclature` (uniq tenant+promotion_id+nm_id+source; source ∈ wb\|excel — excel-строки переживают sync). `sync/tasks_promotions.py` beat 08:30 MSK: list+details (chunk 50) + nomenclatures (non-auto). `/promo-calculator-wb` читает из БД (не дёргает WB на каждый заход), `POST /api/promo-calculator/refresh` — ручной refresh. Загруженный Excel автокомпании persist'ится (source=excel) и показывается при выборе акции; сравнение автоакций работает через excel-строки |
| **0077** | **wb_search_position** — полная выдача поиска WB: наши + конкуренты (DEV-085 follow-up). Расширение шлёт ранг (`POST /api/extension/search-ranking`); анти-спай — пишем только если в выдаче есть наша карточка (`is_own` по join с products). `/jam` секция «Конкуренты по запросу» (`GET /jam/competitor-queries` + `/jam/competitors?query=`). Требует пересборки расширения |
| **0076** | **off_platform_stock_movements.warehouse_name** — мульти-склад своих складов (DEV-083, TS-parity «Склады»). NULL=«Основной». kinds `wh_transfer_out/in` (перемещение между своими складами = пара движений, qty-нейтрально). `services/off_platform.py:transfer_between` + `by_warehouse` в summary. API `POST /api/off-platform/transfer`. UI `OffPlatformStock.tsx`: поле «Склад», карточка «По складам», секция «Перемещение между складами», колонка склада в журнале |
| **0075** | **chart_annotation** — команд-аннотации на дату (DEV-081, TS-parity комментарии-маркеры). `(tenant_id, dt, text, author_name, created_at)`. 📌-маркеры (`ReferenceLine`) на timeseries дашборда за окно графика + панель «Заметки» (add/list/delete). `api/annotations.py`: GET `?from&to` (все роли), POST/DELETE (director_or_head) |
| **0074** | **products.imt_id** (BIGINT+index) — WB склейка (imtID из Content API). DEV-082 «Синхронизация склеек»: `services/skleika_sync.py` тянет nm→imtID, проставляет imt_id, авто-создаёт/обновляет группы `Склейка: <imtID>` (склейки ≥2 nm, идемпотентно). `POST /api/product-groups/sync-skleika` + кнопка на `/product-groups` |
| **0069** | **wb_card_price** — реальная витринная цена покупателя с СПП (TASK-DEV-037 ph3). СПП в seller-API нет → тянем из публичного `card.wb.ru/cards/v4/detail` (без токена, batch nm=a;b, копейки). Composite PK `(tenant_id, nm_id)` без mixin. `basic_price`/`buyer_price` (₽) + `observed_spp_pct=(1−buyer/basic)×100` + `dest` (default Москва 123585712). `integrations/wb/card.py:fetch_card_prices`, `sync/tasks_card_prices.py` beat 05:15 MSK. `/unit-plan` СПП-приоритет: per-row override → **observed (card.wb.ru, >0)** → per-subject map → global default (`unit_plan.GlobalConfig.spp_observed`, `_resolve_spp_pct`) |
| **0082** | **tenants.hidden_at** — скрытие (архив) кабинета WB без удаления данных (DEV-092). NOT NULL → кабинет исключён из `available-tenants`, фильтра «Магазины», свода-по-умолчанию и `sync/tenants.py:get_active_tenants` (диспетчеры пропускают). Возврат = NULL. + backfill BUG-DEV-029: users без записи `user_tenant_access` получают её из legacy `users.tenant_id`/`role` (иначе 403-локаут от middleware) |
| **0086** | **comments** — комментарии-треды (DEV-094, TS-паритет): `(tenant_id, entity_type ∈ kpi\|sku\|warehouse\|rnp_row\|plan\|report, entity_key, body, author_name)`. `api/comments.py`: CRUD + батч-счётчики `/counts`; UI `components/CommentThread.tsx` (💬 N + popover-тред) на KPI-плитках (SummaryTiles), в «Исходной таблице», карточке товара, строках РНП-матрицы, планах, складах. Пишут все кроме bookkeeper; удаляет автор или director |
| **0087** | **rnp_sku_selection** — «Настройки РНП» (DEV-094): `(tenant_id, nm_id, enabled)`, нет строк = показывать все SKU. UI `/rnp-settings` (toggle per SKU, фильтры бренд/категория/поиск, bulk-сохранение). Матрица РНП пересекает выбор с глобальным nm-фильтром (`rnp_matrix.get_rnp_nm_scope`) |
| **0088** | **product_mp_mapping** — «Соответствие товаров» (DEV-094): `(tenant_id, own_sku UNIQUE, nm_id, note)` — маппинг артикула своего учёта на карточку WB. CRUD `/api/off-platform/mappings`, секция на `/off-platform` |
| **0090** | **cogs.vat_rub** — НДС в себестоимости единицы (DEV-096, TS /cost-паритет): Numeric(12,2) NOT NULL DEFAULT 0, справочная (в COGS-расчёт прибыли не входит). UI /cost-history + excel round-trip («cogs» лист +колонка) |
| **0091** | **WMS «Свой склад»** — адресное хранение (DEV-098, Фаза 1). 7 таблиц: `wh_warehouse` (name/code/address, UNIQUE (tenant,name)) · `wh_cell` (code + zone/rack/level/pos, `sort_order` = порядок обхода склада, UNIQUE (tenant,warehouse,code)) · `wh_box` (box_code, brand, supply_ref, `src_no` = колонка `No` из PackingList, status, `cell_id`, `is_mono`, вес/CBM; partial-unique `uq_wh_box_cell` WHERE cell_id IS NOT NULL — «1 ячейка = 1 короб») · `wh_box_item` (barcode/size/`qty_initial`/`qty`, UNIQUE (box_id,barcode)) · `wh_movement` (kind receive/place/relocate/to_storage/pick/ship/adjust/stocktake/wh_transfer_in/out, cell_from/cell_to, append-only) · `wh_barcode_ref` (UNIQUE (tenant,barcode), `source` задаёт приоритет мёржа) · `wh_warehouse_wb_link` (warehouse ↔ `cabinet_tenant_id` × `wb_warehouse_id`; отдельная колонка кабинета, НЕ путать с tenant-скоупом) |
| **0089** | **wb_sync_revision + wb_sync_change** — ревизии WB-отчётов (DEV-095). Revision: `(source ∈ report_detail\|ad_stats\|orders\|sales\|funnel, period_from/to, status running\|done\|error, rows_fetched/added/changed/rejected, totals_delta JSONB, triggered_by beat\|manual\|catchup)`. Change: `(revision_id CASCADE, entity_key ≤128, change_kind added\|updated\|rejected_lower, old/new JSONB — только изменённые tracked-поля)`, index `(tenant_id, entity_key)` для истории строки. Журнал ревизии capped 20 000 изменений (счётчики точные) |
