...
| 0091 | **WMS «Свой склад»** — адресное хранение (DEV-098): `wh_warehouse` (складов несколько, каждый независим) / `wh_cell` (адресуется ТОЛЬКО зона отбора; занятость вычисляется из `wh_box.cell_id`, флага нет) / `wh_box` (+`status` received/pick/storage/shipped/empty, partial-unique `uq_wh_box_cell` = 1 ячейка ↔ 1 короб) / `wh_box_item` (`qty` = текущий остаток, источник истины) / `wh_movement` (append-only журнал) / `wh_barcode_ref` (barcode→nm_id/размер/артикул; приоритет manual > order_file > wb_orders > packing_list) / `wh_warehouse_wb_link` (склад ↔ кабинет × `warehouseId` WB — под отбор по FBS). UI `/warehouse`, `api/warehouse.py`, `services/warehouse/*` |
| 0092 | **WMS: отбор по FBS** (DEV-098 Фаза 3): `wh_fbs_order` — снапшот сборочных заданий WB (`cabinet_tenant_id` = из какого кабинета, `barcode` = `skus[0]`, UNIQUE (tenant,cabinet,wb_order_id)); `wh_pick_order` — лист отбора, **отдельный на кабинет** (+`wb_supply_id` поставки FBS); `wh_pick_line` — строка (ячейка/короб/`qty_required`/`qty_picked`/`shortage`, `sort_order` = копия маршрута на момент генерации). Задания тянутся ТОЛЬКО по кнопке «Собрать отбор» — beat-опроса нет |
...
| `/api/dashboard/period-chart` | director_or_head | **«Период в графике» (DEV-097, как TS)**: дневные серии 8 метрик (продажи/ср.цена до СПП/заказы ₽/ДРРп/логистика/возвраты/чистая прибыль/хранение) + `compare` (prev-период по индексу дня). Блоки Топ-5 маржинальных (pie) и Структура выручки (бары) — фронт из /top-skus и /extended-kpis |
...
| `/api/warehouse/*` | director_or_head | **WMS «Свой склад» (DEV-098)**: склады (CRUD), сетка ячеек (`/cells/upload` формат A, `/cells/generate`, export), приёмка `/receive` — **два формата автоопределением**: `PackingList.xlsx` (`No | Box Code | Barcode | Size | Qty`) и лист «короба» своего склада (`Номер короба | Баркод | Количество | Наименование | Размер`), к любому опц. колонки «Склад»/«Код ячейки»; границы короба по `No`/пустому номеру, а не по `Box Code`; числовой номер нормализуется (`7.0`→`7`); листы «копия/свод/архив» пропускаются (в копии те же номера коробов → перезапись), коллизии номеров попадают в warnings; «Наименование» дозаполняет справочник ШК, `/place` · `/to-storage` · `/boxes[/{code}]`, `/search` (ячейка/ШК короба/баркод/nmID/артикул/название), `/stock` (+`export.xlsx` round-trip с приёмкой), `/movements`, `/barcode-ref` (+`sync-wb`, `import-order` из `ЗАКАЗ №N.xlsx`), `/wb-links`, `/status`; **размещение (Фаза 2)** — `/allocation/preview` · `/apply` · `/export.xlsx` (моно-короба вперёд → сборные greedy set-cover → **пополнение зоны отбора** (`replenish`, по умолчанию вкл.: свободная ячейка + просевший баркод → моно-короб с наибольшим запасом) → хранение; на реальной поставке 451 баркод закрывается 378 ячейками). `/pick-lines/{id}/pick` при опустошении короба возвращает `replacement` — что привезти в освободившуюся ячейку, `DELETE /boxes/{code}` · `/reset-supply` (откат приёмки) · `DELETE /warehouses/{id}?force=true` (**временно на период тестирования** — снимает защиты «есть коробы»/«есть журнал», возвращает и логирует, сколько коробов/штук/записей потеряно). Фото товара в поиске/карте/отборе — `components/ProductThumb` через существующий прокси `/api/products/{nm_id}/photo` (Redis 24ч), nm_id из `wh_barcode_ref`. Мобильное рабочее место кладовщика — `/wh-scan` (режимы «Разместить»/«Найти», камера через общий `components/BarcodeScanner`, режимы «Разместить»/«Отбор»/«Найти»); **отбор по FBS (Фаза 3)** — `POST /pick/collect` (по каждому кабинету его токеном: `/api/v3/orders/new` **и** `/api/v3/orders` за `days_back` дней + `/orders/status` батчем; берутся статусы `new`+`confirm`, потому что задание в поставке выпадает из «новых», хотя не собрано; `supplyId` из ответа подхватывается в лист, фильтр по `warehouseId` из `wh_warehouse_wb_link`, лист на кабинет; `dry_run`), `/pick-orders[/{id}]`, `/pick-lines/{id}/pick` (списывает из короба, пустой короб освобождает ячейку), `/pick-orders/export.xlsx` (лист Excel на кабинет), `/{id}/stickers`, `/{id}/supply` (создать поставку + добавить задания = перевод в `confirm`), `/{id}/supply/deliver` (→ `complete` + QR), `/{id}/cancel`; **остатки FBS (Фаза 4)** — `/fbs-stocks/preview` (в WB / у нас / Δ) и `/fbs-stocks/push` (`PUT /api/v3/stocks`, только по кнопке, только расхождения; режимы `mode=all|fixed|percent` + `value` — весь остаток / по N шт на баркод / P% от остатка, **единица = баркод, не артикул**; проценты вниз, но не до нуля. Чистый расчёт — `services/warehouse/fbs_stock_policy.compute_target`). Клиент WB — `integrations/wb/marketplace.py` (категория лимитера `marketplace`: 120/мин при потолке WB 300/мин, **4XX считается за 10 запросов**; батчи: статусы 1000, стикеры и добавление в поставку — 100, остатки 1000). `services/warehouse/*` |
...
- **Тестировать функционал ТОЛЬКО на проде** (обязательное правило пользователя,
  2026-08-06): `SERVER=vlad@100.90.208.103 ./scripts/remote.sh deploy`, дальше
  smoke через `ssh vlad@100.90.208.103` + `curl http://localhost:4098/...`
  (прод-путь `/opt/rnp`). Локально нет данных и доступов — нет WB-токенов
  кабинетов, не подходят тестовые креды, пуст `wb_orders`; локальный «зелёный»
  результат ничего не доказывает. Локально гоняем только то, что от данных не
  зависит: `pytest` на чистых функциях, `tsc --noEmit` / `vite build`,
  `alembic upgrade` для проверки самой миграции. Свои тестовые сущности на проде
  называть явно тестово и убирать за собой; данные пользователя не трогать.
