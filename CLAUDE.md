...
1. **`FEATURES.md`** — добавить запись в соответствующий раздел (UI / API / сервис).
   Это **single source of truth** по тому что есть в системе (dev-mode).
1a. **`USER_GUIDE.md`** ⭐ — если добавлена/изменена **UI-страница или
   пользовательская функция** — обязательно обновить запись в USER_GUIDE.md
   В ТОМ ЖЕ КОММИТЕ (TASK-LEAD-156, 2026-05-28). USER_GUIDE.md = single source of
   truth для **user-mode** на `/features`. Формат раздела: **Где / Для чего /
   Как использовать 1-2-3 / Пример / Видят**. Без этого каталог `/features` для
   пользователя разойдётся с реальностью (был стейл с мая 2026 до rewrite).
   API-только-изменения (без UI) USER_GUIDE не требуют.
...
| `/api/leak-report` | director_or_head (brands-filter урезает) | **Leak-report «найдено N₽»** (TASK-LEAD-140): аудит-артефакт кабинета за период — одно число «найдено» + breakdown по 5 источникам (оспоримые штрафы → вернуть; минусовые SKU / дохлый сток в платном хранении / переплата логистики из-за перемеров WB / убыточные акции постфактум → прекратить) + recon trust-badge «сверено с WB до рубля». Reuse: `unit_economics`, `chargebacks`, `pnl_reconciliation`, `WbProductDimensionsHistory×WbTariffBox`, `wb_report_detail.seller_promo_*`. UI `pages/LeakReport.tsx` — печатный/PDF-вид (`window.print()`). Ритуал входа в клуб + sales-артефакт. См. `services/leak_report.py`. |
