/**
 * TASK-LEAD-053 — Калькулятор стоимости обычной поставки на WB-склад.
 * TASK-LEAD-077 (2026-05-22) — переименован из TransitCalculator → SupplyCalculator.
 * Транзитный калькулятор живёт отдельно в `TransitCalculator.tsx`.
 *
...
const SUPPLY_KEY = "supply-calc.params.v1";
const LEGACY_KEY = "transit-calc.params.v1"; // обратная совместимость со старым ключом
...
function loadParams(): SavedParams {
  try {
    const raw =
      localStorage.getItem(SUPPLY_KEY) ?? localStorage.getItem(LEGACY_KEY);
    if (raw) {
...
function saveParams(p: SavedParams) {
  try {
    localStorage.setItem(SUPPLY_KEY, JSON.stringify(p));
  } catch {}
}
      ...
      <PageHeader
        ...
        subtitle={
          <>
            ...
            N штук на конкретный WB-склад напрямую. Использует текущие WB-тарифы
            (миграция 0040, обновляется ежедневно 08:00 MSK). Для расчёта поставки
            через хаб → конечный склад смотри{" "}
            <a className="text-accent" href="/transit-calculator">
