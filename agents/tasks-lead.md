...
### TASK-LEAD-154: UI batch — /localization top-N до 500, убрать «Отчёт менеджера», переименовать промо-калькулятор
- **Приоритет:** P2 (smoke 2026-05-28, мелкие UX-правки)
- **Источник:** Несколько запросов пользователя подряд:
  1. На `/localization` «Top-N SKU с худшей локализацией» — селектор макс 50,
     добавить 100/200/300/400/500.
  2. Убрать секцию «Отчёт менеджера» в шапке `/weekly-report` (не нужна).
  3. Переименовать `/promo-calculator` «Калькулятор рентабельности WB-акций»
     → «Калькулятор рентабельности» (generic). Под названием
     «Калькулятор рентабельности WB-акций» поедет новая страница с
     WB-промо API (TASK-LEAD-155, отдельно).
- **Затронуто:** `pages/Localization.tsx` (+5 options), `pages/WeeklyReport.tsx`
  (удалён header-блок «Отчёт менеджера»), `pages/PromoCalculator.tsx` (title),
  `components/Layout.tsx` (sidebar label).
- **Статус:** Выполнено — 2026-05-28

### TASK-LEAD-156: USER_GUIDE rewrite — все ~50 фич в простом формате «Для чего / 1-2-3 / Пример»
- **Приоритет:** P1 (smoke 2026-05-28 — фрустрация юзера: «описания сложные и не обновляются»)
- **Источник:** На `/features` в режиме «User-mode» рендерился `USER_GUIDE.md`,
  который содержал только 9 разделов (из ~50 фич) и был написан техническим
  языком. Пользователь хотел простые описания в формате «использовать для X,
  применить 1-2-3», и чтобы документация не отставала от кода.
- **Решение:**
  - Полный rewrite `USER_GUIDE.md` — все ~50 страниц/функций сидбара покрыты
    единым форматом: **Где (меню → раздел) / Для чего (одно предложение) /
    Как использовать (1-2-3 шага) / Пример (markdown table или сценарий) /
    Видят (роли)**.
  - Группировка по сидбар-секциям: Обзор / Налоги и деньги / SKU и продажи /
    Маркетинг / Финансы внутренние / Контроль / Справка / Админка.
  - Добавлено обязательное правило в CLAUDE.md §«Обновить документацию»: при
    UI-изменении обновить USER_GUIDE в том же коммите. Это решает причину
    стейла (с мая 2026 файл не обновлялся).
- **Затронуто:** `USER_GUIDE.md` (полный rewrite, 653 → ~700 строк),
...
### TASK-LEAD-159: Сжатие CLAUDE.md — убрать дубли, оставить индекс + инварианты
- **Приоритет:** P2 (chore, эффективность контекста)
- **Источник:** запрос пользователя «оптимизировать файлы, занимающие контекст».
  В контекст каждой сессии авто-грузится только `CLAUDE.md` (был 799 строк /
  ~75KB) + `MEMORY.md`. Бóльшая часть `CLAUDE.md` дублировала файлы, читаемые
  по запросу (детали миграций → FEATURES.md, процессные правила → RULES.md,
  WB-лимиты → WB_API_REFERENCE.md).
- **Сделано:**
  - `CLAUDE.md` ужат 799 → 388 строк (~75KB → ~30KB, −59%). Превращён в плотный
    индекс + инварианты, нужные всегда. Сохранены дословно: 16 подводных
    камней, RBAC-таблица, API-таблица (компактнее), period_aggregates,
    reporting_mode, release-цикл, UI-conventions, pre-flight, permissions.
    Свёрнуты до 1 строки + ссылки: эссе миграций 0048-0063, детали портов
    A/B и Chrome-extension, multi-cabinet backward-compat.
  - `FEATURES.md` → секция «Миграции» дополнена недостающими 0048-0054 и
    0056-0063 (была стейл до 0055), заголовок `(0001–0063)`. Теперь ссылка
    из CLAUDE.md «детали → FEATURES.md» честная.
- **Критерии готовности:**
  - [x] CLAUDE.md ≤ 400 строк, все behavior-инварианты сохранены
  - [x] Все §-ссылки на RULES.md (2.5-2.9, 11) и FEATURES.md актуальны
  - [x] Ни одна миграция 0001-0063 не потеряла детали (есть в FEATURES.md)
- **Статус:** Выполнено — 2026-05-28
