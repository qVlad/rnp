...
- **Прочие удержания — остаток 6 902:** на проде «Прочие удержания» = 29 892
  (целиком операция «Удержание», penalty=0): строки 05-21 = 22 990 и 05-24 = 6 902.
  TS otherDeduction = **22 990** (только 05-21). Строку 05-24 (6 902) TS НЕ относит
  к otherDeduction (проверено: d24 otherDeduction=0; в след. неделю не уносит).
  bonus_type_name обеих строк в БД пуст → добавили bonus_type_name/doc_type_name в
  `/api/operations` для диагностики. ❗Пока наш prochie=29 892 → profit ниже TS на
  6 902 (474 191 vs 482 206). Точное правило исключения «Удержание» — TODO.
...
- [ ] ❗Прочие удержания — найти правило, по которому TS исключает часть строк
      «Удержание» (6 902 на 05-24 → TS 0). Смотреть bonus_type_name в /operations.
