...
### TASK-DEV-098: WMS «Свой склад» — адресное хранение, размещение, поиск/остатки, отбор по FBS — Выполнено — 2026-08-06 (v0.94.1)
- **Тип:** feature (крупная, 4 фазы) · запрос пользователя 2026-08-03: система управления
  своим складом — загрузка мест хранения (ячейка = короб 60×40×40), содержимое занятых
  ячеек в формате `PackingList.xlsx`, подсказка «какой короб в какую ячейку» с приоритетом
  моноартикульным коробам, остальное на хранение; далее быстрый поиск и остатки.
- **Согласовано с пользователем:** складов несколько и каждый независим · адресуется
  только зона отбора (хранение — без адреса) · 1 ячейка = 1 короб · «файл заказа» =
  тот же PackingList · приоритет строго моно-короба вперёд · справочник ШК — новая
  таблица (авто из `wb_orders`/`wb_stocks` + импорт `ЗАКАЗ №N.xlsx`) · сканирование —
  телефон + камера (переиспользуем `html5-qrcode` с `/box-scan`) · tenant-scoped в
  основном кабинете + поле бренда · **отбор из FBS-заказов WB по кнопке**, лист на
  каждый кабинет, кабинетов 4–5 · пуш остатков FBS в WB — по кнопке с preview ·
  в системе делаем стикеры заданий, статусы confirm/complete, поставку FBS + QR.
- **Данные PackingList (проверено на реальном файле):** 822 физических короба (границы
  по колонке `No`, а не по `Box Code` — 6 коробов идут с `Box Code = «—»`), 451
  уникальный баркод, 565 моно / 257 сборных, Σqty 56 983. **Моно-короба покрывают только
  287 из 451 баркода** — остальные 164 есть только в сборных, поэтому greedy-шаг по
  сборным коробам обязателен.
- **WB FBS API (изучено по OpenAPI из wildberries-sdk, `dev.wildberries.ru` отдаёт 498):**
  хост `marketplace-api.wildberries.ru`, лимит 300 req/мин **на кабинет** (200 мс,
  всплеск 20, 4XX = 10 запросов), `DELETE /stocks` — 10/мин. В `GET /api/v3/orders/new`
  **`skus[]` = баркоды**, т.е. прямой матч с `wh_box_item.barcode`; 1 задание = 1 единица;
  `warehouseId` = склад продавца (у каждого кабинета свой на тот же физический склад).
- **Фазы:** (1) миграция 0091 + склады/ячейки/коробы/движения/справочник ШК + приёмка +
  поиск/остатки + `/warehouse`; (2) алгоритм размещения + `/wh-scan`; (3) миграция 0092 +
  `integrations/wb/marketplace.py` + отбор по FBS + стикеры/статусы/поставка;
  (4) пуш остатков FBS, пополнение pick-face, капитализация.
- **Фаза 1 сделана (v0.92.0…0.92.3):** миграция 0091 (7 таблиц) · `services/warehouse/*`
  (packing_list / cells / order_file / barcode_ref / receive / stock / movements / excel) ·
  `api/warehouse.py` · страница `/warehouse` (7 табов) · 14 тестов парсеров.
- **Смоук на проде 100.90.208.103 (реальный PackingList):** 822 короба за 2.8 с,
  451 баркод, 56 983 шт — сошлось 1:1 с эталоном; справочник ШК 327 новых + 124
  не перезатёрты (приоритет источников работает); конфликт ячейки → 409; поиск по
  названию «сорочка» → 95 строк / 7 699 шт; выгрузка состояния → повторный разбор
  без расхождений; откат поставки и удаление короба — ок. Тестовые данные убраны.
- **Что смоук нашёл и что исправлено:** (1) `sync_from_wb` брал артикул только из
  `products`, а у кабинета там 2 строки против 619 баркодов в `wb_orders` → теперь
  артикул/предмет читаются из строк заказов (v0.92.1); (2) не было отката приёмки →
  `DELETE /boxes/{code}` + `POST /reset-supply` (v0.92.2); (3) удаление склада
  каскадом стирало append-only журнал (1126 → 0) → 409 `warehouse_has_history:N`,
  штатный путь — снять «Активен», осознанно — `?force=true` (v0.92.3).
- **Фаза 2 сделана (v0.93.0):** `services/warehouse/allocation.py` — моно-короба
  вперёд → сборные greedy set-cover → хранение; `/allocation/preview|apply|export.xlsx`;
  `components/BarcodeScanner` (вынесен из `BoxDistribution.tsx`) + мобильная
  `/wh-scan`. На реальной поставке 451 баркод закрывается **378 ячейками** (287
  моно + 91 сборный короб на остальные 164 баркода) — greedy экономит 73 ячейки.
  12 тестов алгоритма (инварианты + детерминизм + 2 эталонных на реальном файле).
- **Фазы 3-4 сделаны (v0.94.0…0.94.1):** миграция 0092 (`wh_fbs_order` /
  `wh_pick_order` / `wh_pick_line`), `integrations/wb/marketplace.py` + категория
  лимитера `marketplace`; отбор по FBS (кнопка → `/orders/new` по кабинетам →
  фильтр по `warehouseId` из связок → **лист на кабинет** → маршрут → списание,
  `shortage` при нехватке); стикеры → поставка FBS → в доставку + QR; сверка и
  пуш остатков FBS (`preview` / `push`, только расхождения, только по кнопке);
  подтягивание складов продавца `GET /api/v3/warehouses` для связок.
- **Что уточнилось по WB API (записано в `WB_API_REFERENCE.md` § Marketplace):**
  (1) отдельной ручки `confirm` у задания НЕТ — задание переходит в `confirm` при
  добавлении в поставку, в `complete` при её отправке; (2) батчи не одинаковые:
  статусы 1000, но **стикеры и добавление в поставку — 100**; (3) остатки FBS
  читаются POST-ом, а не GET; (4) 4XX учитывается как 10 запросов, поэтому
  лимитер держим на 120/мин при потолке 300/мин.
- **Проверено на живом WB (прод, 2026-08-06):** `GET /api/v3/warehouses` отдал
  склады продавца по обоим подключённым кабинетам (ООО Альтеком → 1913922,
  Таланов СС → 1953622); `pick/collect?dry_run=true` и `fbs-stocks/preview`
  прошли без ошибок. Сборочных заданий на момент проверки не было (0), поэтому
  разбор реального payload'а задания на живых данных ещё не проверялся —
  проверить при первых FBS-заказах.
- **Найдено при тестировании пользователем на проде (v0.94.2…0.94.3):**
  (1) временное `?force=true` на `DELETE /warehouses/{id}` — снести склад вместе с
  коробами (по запросу; когда тестирование закончится — убрать ветку force для
  коробов, оставив её только для журнала);
  (2) **«Не покрыто» врало** — метрика считала покрытие только по коробам на
  хранении и игнорировала уже стоящие в ячейках: на складе ООО2 показывало 354
  при реально отсутствующих **70** (всего 451 баркод, в отборе 381, на хранении
  354, пересечение 284). `plan_placement` получил `already_covered`; такие
  баркоды не попадают в «не покрыто» и не тратят ячейки;
  (3) **`sort_order` сталкивался между зонами** — `generate_cells` нумерует
  только свою сетку, поэтому зоны A и B получили одинаковые 10,20,30… и обход шёл
  A-01-01-01 → B-01-01-01 → B-01-01-02 → A-01-01-02. Добавлен
  `resequence_warehouse_cells` (авто после загрузки/генерации + кнопка
  «Пересчитать маршрут» для складов, созданных до фикса);
  (4) UX: при 0 свободных ячеек вкладка «Размещение» выглядела как «размещение не
  сработало» (пустой маршрут) — добавлено пояснение.
- **Дозаказы пользователя при тестировании (v0.94.5…0.96.0):**
  (5) печать стикеров блокировала браузер — окно печати открывалось даже при
  пустом ответе WB, `print()` вызывался до загрузки картинок (v0.94.5);
  (6) **фото товара** по баркоду в поиске/карте/отборе — `components/ProductThumb`
  через существующий прокси `/api/products/{nm_id}/photo`, nm_id из `wh_barcode_ref`
  (v0.95.0);
  (7) **пополнение зоны отбора** (v0.96.0) — шаг 3 алгоритма: свободная ячейка +
  самый просевший баркод → моно-короб с наибольшим запасом; флаг `replenish`;
  `/pick-lines/{id}/pick` отдаёт `replacement` и сканер сразу говорит, что
  привезти в освободившуюся ячейку. Режим «переразложить с нуля» пользователь
  **отклонил** — нужно было именно пополнение.
- **Разъяснено пользователю (не баги):** разное размещение на складах = разное
  число свободных ячеек на момент «Применить» + алгоритм останавливается при
  полном покрытии ассортимента (два preview подряд идентичны побайтово, проверено
  на 4 складах); маршрут A→B корректен (одна смена зоны, коллизий 0); недостача в
  листе отбора на тестовых данных — ожидаемо, данные не настоящие.
- **План:** `/Users/user/.claude-work/plans/floating-hatching-alpaca.md`

### TASK-DEV-097: TS-графики на дашборде — «Период в графике», Топ-5 маржинальных (pie), Структура выручки (бары) — Выполнено — 2026-07-05 (v0.91.0)
- **Тип:** feature · запрос пользователя 2026-07-05 (скриншот TS): на главной нет
  TS-блоков «Период в графике» (мультиметричный график: Продажи/Ср.цена до скидок
  МП/Заказы ₽/ДРРп/Логистика/Возвраты/Чистая прибыль/Хранение, чекбоксы, сравнение
  с прошлым периодом, комментарии-маркеры), «Топ 5 маржинальных товаров» (pie с
  долями + остальные), «Структура выручки» в TS-виде (горизонтальные бары со
  всеми статьями и суммами).
- **Механика:** новый `GET /api/dashboard/period-chart?start_date&end_date&compare`
  (director/head): серии по дням из report_detail (period_aggregates-предикаты:
  sales/realisation/qty/логистика/хранение/возвраты) + wb_orders (заказы ₽) +
  wb_ad_stats_daily (ДРРп) + build_pnl(day) (чистая прибыль); compare — те же
  серии за прошлый период. Pie и бары — фронт-only из /top-skus?by=margin и
  /extended-kpis totals. Компоненты PeriodChart/TopMarginPie/RevenueStructureBars,
  замена блока «Динамика выручки» на Dashboard, 📌-аннотации как маркеры.

### TASK-DEV-096: Независимая сверка 1:1 с TrueStats + пакет паритета 2 — Выполнено — 2026-07-04 (v0.90.0)
- **Тип:** audit + bugfix · запрос пользователя 2026-07-04: «ещё раз независимо
  проверить что 1 в 1 truestats, может перенести одинаковые разделы в /truestats».
- **Метод:** живой обход mirror-app.truestats.ru (все 17 разделов) vs прод
  rnp.sellerfriends.ru через Chrome DevTools (структура: виджеты/вкладки/колонки).
- **Итог паритета:** дашборд 39/39 виджетов ✓, Исходная таблица ✓, ДДС/Операции/
  Дополнительно (5 вкладок вкл. Настройки) ✓, /files ✓, план-факт ✓, heatmap ✓,
  удержания ✓, группы-склейки ✓. Найденные гэпы: (1) сводный отчёт «По неделям»
  (строки-недели), (2) постатейный OPEX из финсправочника в P&L, (3) расширенная
  аналитика РК (зоны показа, корзина-воронка, CPL/CPS per-кампания), (4) minor:
  НДС-колонка в себестоимости, аналитика по размерам в карточке, период
  усреднения прогноза РНП 7/28. TS /tariffs и реферальный кабинет — их биллинг,
  не переносим. Ассоц. заказы — нет в публичном WB API (честно скрыто).
- **Bugfix:** BUG-DEV-030 — /api/rnp/matrix 500 (label("t") vs Row.t SQLAlchemy) +
  error-стейт RnpModule (v0.89.2).
- **Решения пользователя (2026-07-04):** профиль меню «TrueStats» + роут
  /truestats (вариант 1); закрыть ВСЕ гэпы: недельный свод, постатейный OPEX,
  расширенная Аналитика РК, мелочи (НДС в COGS — миграция 0090, размеры в
  карточке, окно прогноза РНП 7/28, импорт COGS на странице). Один релиз v0.90.0.
- **Итог:** все блоки реализованы — /truestats (UX-профиль truestats в Layout),
  `/api/summary-report/weekly` + WeeklySummaryTable (Redis-кэш закрытых недель),
  `/api/pnl/opex-breakdown` + раскрытие в PnL.tsx, `/api/ad-campaigns/analytics`
  расширен (зоны/воронка/CPL/CPS/цены/остатки, `/daily` drill, новая страница),
  карточка товара (+размеры/+склады), `/api/rnp/forecast-window` (7/28/period),
  миграция 0090 cogs.vat_rub + импорт на /cost-history.
...
- **Найдено при аудите:** 5 из 6 «открытых» гэпов старого аудита уже были
  сделаны (inventory/compare/plan_distribute/PWA/contribution_margin) —
  реальный список гэпов собран по живому TS. Ассоциированные заказы TS — в
  публичном WB API отсутствуют (fullstats их не отдаёт), строка честно не
  показывается; вернуться, если WB откроет поле.
- **✅ Итог (v0.88.0, 3 деплоя: релиз + 2 фикса сверки):** миграции 0086-0088;
  pytest на проде 27 новых / 299 passed (10 failed — прежние pre-existing);
  API-смоук всех эндпоинтов. **Сверка с живым TS (Onyx, 22-28.06):**
  реализация/продажи/факт-комиссия/эквайринг/цены/штрафы/номинальная комиссия
  (214 148,99 ₽, 33.75%) — копейка-в-копейку; заказы Δ0.3% (лаг синка Воронки);
  формула GMROI совпала. Уроки: (1) TS «Заказы» = Воронка ordersCount, не
  Лента; (2) номинальная = paid_storage_kgvp + ЗНАКОВАЯ поправка конфига
  unit-plan; (3) % выкупа на историч. неделях ниже TS — WB доначисляет buyouts
  задним числом, наш rolling-7 FREEZE не догоняет (ограничение WB, как
  orders-funnel-limit); (4) вознаграждение ВБ Δ~4% — документная метрика
  (TASK-DEV-061). Капитализация — снапшоты наши ≠ TS by-design.
...
- **Решения пользователя:** банк-API (Т-Банк) — позже; старая ДДС — вкладкой;
  всё одним релизом.
- **✅ Итог (v0.87.0):** задеплоено (первый деплой упал: 0083 дублировала индекс
  `ix_manual_operation_tenant_date` из 0071 — исправлено, alembic на 0085).
  pytest на проде: 21/21 новых+мульти-кабинет, 293 passed всего (10 failed —
  прежние pre-existing). API-смоук в скрытом кабинете 151: балансы/переводы/
  сальдо копейка-в-копейку, выписка 1С cp1251 импортирована, повторная
  загрузка → 0 imported / 2 skipped (дедуп), автоправило проставило статью
  (applied_rule_id), старый `GET /api/cash-flow` не изменился
  (net=pnl_cash_flow). UI-смоук: /cash-flow (матрица+вкладки), /operations
  (лента+импорт+календарь), /finance-extras (5 вкладок). Активный кабинет
  vipryn возвращён на tenant 1.
- **Backlog (отдельные задачи):** Т-Банк API (bank_meta готов); свод «Магазины»
  в ДДС-матрице; распределение операций по кабинетам/брендам; размазывание
  операции по периоду (spread); beat-задача для авто-плана WB (сейчас — кнопка
  и toggle).
...
> ЭДО Диадок, мультивалютность — мы single-seller WB. Реальные гэпы → DEV-079…086.

### TASK-DEV-080: Тепловая карта — двойная метрика (цвет + размер) — P2 ✅
- **Тип:** feature/parity · TS «Тепловая карта» dual-metric. ✅ **ВЫПОЛНЕНО 2026-06-13 (v0.64.51)**.
- Добавлен селектор «Размер ячейки» (2-я метрика): цвет ячейки = основная метрика,
...
- **Тип:** feature/parity · 🟡 **ЧАСТИЧНО ВЫПОЛНЕНО 2026-06-13 (v0.71.0)**.
- **✅ Динамика позиций:** данные ЕСТЬ — `abtest_position_snapshot` (nm_id, query,
  position, page, collected_at; шлёт расширение при заходе на поиск WB). Эндпоинт
...
### TASK-DEV-091: Мобильный QR-сканер раскладки коробов — Выполнено — 2026-06-21 (v0.81.0)
- **Тип:** feature · запрос пользователя 2026-06-20. План: `.claude-work/plans/ticklish-squishing-pie.md`.
- **Что:** мобильная страница `/box-scan` (вне Layout, камера html5-qrcode). Скан QR
  входящего короба (ШК `ALT-...`) → подсказка раскладки по складам в WB-короба
  (`WB_1541505000++`, **накопительная** модель: один открытый короб на склад, копится
  из разных сканов, «Заполнено» закрывает и открывает новый). Ручная правка кол-ва,
  «Распределено» (входящий короб готов). Экспорт shk-excel.
- **Backend:** миграция 0080 (`box_distribution_src/_wb_box/_wb_item`),
  `api/box_distribution.py`, `services/box_distribution.py` (парсер 3 листов Ink/Ld/Lk
  + сборка xlsx). Счётчик WB и карта алиасов складов — в AppSetting (tenant-scoped).
  Нормализация: баркоды без `.0`, склады канонизируются (Шушары/Питер/СПБ→СПб,
  Перспективная 14/17→Екатеринбург-Перспективная; редактируемая карта в /settings).
- **Frontend:** `pages/BoxDistribution.tsx`, `components/BoxDistributionSettings.tsx`
  (загрузка+слияние складов в /settings), пункт меню «Контроль», dep `html5-qrcode`.
- **Камера требует HTTPS** (https://rnp.sellerfriends.ru), на http баннер-предупреждение.
- **✅ Доработки (v0.82.0, миграция 0081):** distributed_qty — запрет повторной
  раскладки + остатки при частичной (cap на остаток); «Заполнено» перенесён в
  список WB-коробов; убран ручной ввод ШК; список распределённых коробов
  (`/distributed-boxes`); экспорт +колонка «Товар с кизом»=да (шаблон page-excel);
  `POST /reset?confirm=true` + кнопка с двойным подтверждением. Проверено на проде.
- **✅ WB-диапазоны по городам (v0.83.0):** `_next_wb_code` city-aware — WB-номера
  выдаются из per-city диапазона (`PUT /wb-ranges`, по 300 с WB_1541505000),
  `/wb-ranges.xlsx` (WB→город), кнопка в /settings. Применены распределения из 4
  файлов (топы/инкантина/платья-юбки/кидс) → `Копия Распределение ООО
  (перераспределено).xlsx`; инкантина по пропорциям (план>упаковано). Проверено.

### TASK-DEV-088: /promo-margin — источники скидок (WB/файл) + сравнение акций — P2
- **Тип:** feature · запрос пользователя 2026-06-17.
- **✅ Симулятор одной акции с источниками (v0.77.0):** на `/promo-margin` вкладка
  «Симуляция одной акции» — источник скидок: ручная единая % / выбранная WB-акция
  (per-SKU цены из кэша `wb_promotion_nomenclature`) / загруженный Excel акции
  (реюз `parse-promo-file`). Скидка%/SKU = (current−promo)/current, применяется к
  плановой цене /unit-plan той же относительной величиной (basis-независимо).
- **✅ Сравнение до 3 акций (v0.77.0):** вкладка «Сравнение акций» — 3 слота
  (каждый WB/файл/ручная), таблица per-SKU «маржа ДО → после по каждой акции»,
  зелёным — лучшая акция по SKU; футер Σ маржа/шт + «выгоднее в N SKU».
- **Backend:** `_compute_unit_plan_rows` обобщён на per-SKU скидки
  (`promo_discount_by_nm`, clamp [0,99]); новый `POST /api/unit-plan/promo-margin`
  (discount_pct | discount_by_nm + nm_ids); item += `promo_discount_pct`.
  Метрики — только маржа/шт (без «целиком»-прогноза, по выбору пользователя).

### TASK-DEV-087: Маржа в акции (до/после) + buyout из Воронки — P2
- **Тип:** feature + data/parity · запрос пользователя 2026-06-15.
- **✅ Страница `/promo-margin` (v0.73.0):** копия идеи promo-calculator-wb, но
  фокус — маржа/шт ДО vs ПОСЛЕ вступления в акцию (₽ и %), baseline 1:1 из
  `/units` (unit-экономика). Модель: комиссия %-масштабируется с ценой, остальные
  затраты фиксированы ₽. `PromoMargin.tsx`, без нового бэкенда (реюз `/api/units`).
- **✅ % выкупа из Воронки (v0.73.0):** `_buyout_pct_30d` → funnel-primary
...
- **✅ Приёмка/ИРП из факта (2026-06-16):** по запросу пользователя приёмка
  (`acceptance_rub_per_liter`) и `irp_coef` тоже авто-подтягиваются из факта
  (`compute_recommended_coefs`: +`acceptance_rub_per_liter_actual` =
  Σpaid_acceptance/Σ billable-литров). 0 валиден (приёмка не начислялась →
  план=факт), None (нет строк) → ручной дефолт. ⚠️ Приёмка заходит в план дважды
  (строка AS 1.70₽/л + член ИРП цена×0.017) — пользователь подтвердил, что это
  два разных расхода (так задумано в Excel-эталоне). Оба тянутся из одного
  `paid_acceptance` — при ненулевой приёмке будет ×2; сейчас факт=0, оба 0.
