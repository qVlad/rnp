...
### TASK-DEV-098: WMS «Свой склад» — адресное хранение, размещение, поиск/остатки, отбор по FBS — Фаза 1 выполнена — 2026-08-06 (v0.92.3), Фазы 2-4 в работе
...
- **Осталось (Фазы 2-4):** алгоритм размещения (моно вперёд → greedy по сборным →
  хранение) + мобильная `/wh-scan`; миграция 0092 + `integrations/wb/marketplace.py`
  + отбор по FBS (кнопка → `/orders/new` по 4-5 кабинетам → лист на кабинет →
  маршрут → списание) + стикеры/статусы/поставка FBS; пуш остатков FBS по кнопке.
