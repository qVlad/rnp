...
- **Тип:** feature/parity · PARTIAL (часть может быть data-blocked). TS поиск-аналитика:
  50+ метрик, кластеризация (✅ у нас есть), сравнение с конкурентами по тем же
  запросам, динамика позиций по ключам. У нас `/jam` ~13-17 метрик, кластеры есть,
  НЕТ конкурентного сравнения и динамики позиций (нет полей позиций в JamQuery).
