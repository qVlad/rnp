...
- **Тип:** parity/feature · **P1** · детальная сверка пользователя 25-31.05. Эталон TS
  ...
  - **❌ Выпадашка логистики (5 детализаций) — НЕ сделана (data gap):** TS делит
    логистику на 5 (К клиенту при отмене/продаже, От клиента при отмене/возврате,
    Возврат брака). У нас вся delivery_rub под одной операцией «Логистика» — WB-под-
    разбивки (direction/тип доставки) НЕ сохраняем в wb_report_detail. Нужно: добавить
    поле классификации логистики при синке report_detail. Отдельная подзадача.
