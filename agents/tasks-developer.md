...
- **Тип:** mismatch · **P1** — после v0.64.5/6 сверены ВСЕ плитки vs TS-скрин.
  ...
  3. **Прибыль** — теперь вычитает «Прочие удержания» (операционные, БЕЗ штрафов),
     аллокация по SKU пропорц. реализации (как OPEX). Для 18-24: 504 083 − 22 990 =
     481 093 ≈ TS 482 206 (Δ1113 = реклама 806 + налог 307). Штрафы в прибыль НЕ
     входят (отдельная строка) — соответствует TS. Маржа = прибыль/реализация
     подтягивается автоматически.
- **Урок:** проверять КАЖДУЮ плитку числом, а не только слой данных. Дашборд-mapping
  был неверный (TS-сетка = наш Сводный отчёт, не Дашборд). RCA «неполных заказов» —
  не WB rate-limit, а логика «частичное покрытие funnel принималось за полное».
