...
- **Контекст:** ЖИВАЯ сверка (api2.truestats.ru acc 25143 ↔ наш прод financial-режим),
  ...
  **База сходится копейка-в-копейку** (независимо подтверждено, не круговой проверкой):
  ...
  - **TASK-DEV-069 (P1, данные):** `ad_cost=0` у нас vs TS `advertisingExpense=18 900`
    за 18-24.05 — наш `wb_ad_stats_daily` за неделю затёрт в 0 (реклама-волатильность,
    pitfall #18a: freeze «не понижать ненулевое нулём» либо не сработал, либо данные
    потеряны до freeze). Это главный драйвер расхождения profit. Фикс: разобраться
    почему обнулилось + freeze/backfill ad spend. Память `ad-data-volatility`.
