...
- **Критерии готовности:**
  - [ ] Фильтрация работает client-side (не нужен новый API)
  - [ ] Подсветка рядов через CSS class по dataset-условию
  - [ ] Пресеты сохраняются и видны при следующем заходе
- **Зависимости:** нет
- **Статус:** Открыта
...
- **Критерии готовности:**
  - [ ] Backend: `GET /api/supply/recommendations.xlsx` или генерация на фронте
        через xlsx-lib (предпочтительно фронт — данные уже есть)
  - [ ] Колонки readable: «Артикул», «Бренд», «Склад», «К отгрузке (шт)»,
        «Остаток», «Дн.до стокаута»
- **Зависимости:** нет
- **Статус:** Открыта
...
- **Описание:** В `AlertsBar` добавить mark-as-read через localStorage
  ...
  основном списке.
- **Критерии готовности:**
  - [ ] Ack хранится в `localStorage["alerts.ack.v1"]`, key=hash(alert.id+date)
  - [ ] Кнопка «✓ прочитано» рядом с каждым алертом
  - [ ] Раздел «Прочитанные сегодня» — collapsed, можно вернуть в основной
- **Зависимости:** нет
- **Статус:** Открыта
