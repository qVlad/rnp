...
- **Тип:** bug/ops · **P1**
- `sync_stocks` (beat 06:30/18:30) не обновлял остатки с 20.05 при живых
  orders/sales. Manual-refresh сработал (→05.06). Разобрать почему beat молчал
  (worker-stats очередь / расписание / тихая ошибка).
...
- ✅ DEV-056 реклама — TS НЕ вводит вручную (WB advert API); починен wipe-баг
  `sync_ad_stats` (удалял весь диапазон при 429). Данные восстановятся при fetch.
