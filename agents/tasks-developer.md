...
- **РЕШЕНО:** построен парсер листа «Отчеты+УПД» (`parse_stas_razmetka_xlsx` +
  ...
  Импортирован файл на прод (бэкап `pre-ausn-import-2026-06-11-1130`): **апрель
  база 4 461 677.89 / налог 356 934.23 (6 заявок); май база 7 254 066.50 / налог
  580 325.32 (8 заявок, УПД доставки 4 514.14)** — копейка-в-копейку с файлом.
  Дальше бухгалтер сам перезаливает файл через `/taxes` (импортёр идемпотентен).
