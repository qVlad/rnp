...
- **Тип:** mismatch · **P1** — после v0.64.5/6 сверены ВСЕ плитки vs TS-скрин.
  ...
  ❌ Доделать:
  1. **Заказы / % выкупа** — наши preliminary-данные неполны (314 vs TS 871;
     wb_orders/funnel за старые недели с пропусками из-за WB rate-limit). Нужна
     полнота истории заказов/воронки (связано с DEV-055/056 sync-надёжность).
  2. **Маржа %** — формула исправлена (прибыль/реализация), но зависит от прибыли;
     расходится на величину residual (26.14 vs 25.01).
  3. **Прочие удержания** — наш scope (29 892) включает штрафы; TS otherDeduction
     (22 990) их разделяет. Убрать penalty из «прочих», вынести штрафы отдельно.
  4. **Прибыль** — TS вычитает «Прочие удержания» в прибыль (для 18-24 profit =
     наш − prochie), но для 12-18 — нет (там «Удержание» 26k не вычитался). Методика
     TS непоследовательна/неясна — выяснить и привести профит к 1:1.
- **Урок:** проверять КАЖДУЮ плитку числом, а не только слой данных. Дашборд-mapping
  был неверный (TS-сетка = наш Сводный отчёт, не Дашборд).
