...
## Правило 2.8 — Claim перед правкой (anti-race для параллельных сессий)
...
он защищал только сам деплой, не код.

**Решение.** `agents/CLAIMS.md` (механика) + `scripts/claim.sh` (CLI).
Каждая задача, требующая > 30 мин работы или резервирующая «горячий ресурс»
(нумерация задач, миграций, version-файлы), сопровождается claim'ом.

### Когда брать claim

Обязательно:
- TASK-NNN ставится в `В работе` (резервируется номер)
- Создаётся новая миграция (резервируется N)
- Bump SemVer в трёх файлах (но это уже делается через `scripts/bump.sh` —
  он атомарный)
- Длительная (> 30 мин) правка многих файлов

Опционально / не нужно:
- Чтение кода / документов
- Hotfix < 5 мин в одном файле
- Markdown-правки документации
...
Claim — markdown в `agents/claims/<task-id>.claim.json` под git. Атомарный
