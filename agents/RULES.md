...
## Правило 2.6 — Deploy lock (координация деплоев)

**Перед любым деплоем на прод** — обязательно проверить `DEPLOY_LOCK.md` в
...
30 мин). Два параллельных деплоя из разных сессий ломают друг друга: один
делает rsync поверх другого, build конфликтует, состояние БД в неопределённое.
Файловый замок — простая координация без внешних систем.

### Workflow

1. **Открыть `DEPLOY_LOCK.md`** перед `./scripts/remote.sh deploy`.
2. **Если статус `🟢 Свободно`** — заменить блок «Статус» по шаблону
   (`🔴 Занято` + кто + время MSK + что катит + версия + ожидаемая
   длительность), закоммитить эту правку **отдельным коммитом**
   (`chore(deploy): lock — vX.Y.Z TASK-NNN`), запушить.
   *Только после этого* запускать деплой.
3. **Если статус `🔴 Занято`** — НЕ деплоить молча. Спросить пользователя:
   «`DEPLOY_LOCK.md` помечен `🔴 Занято` (кто=X, с=Y MSK, катит=Z) —
   реально нужно перебить деплой?». Только после явного «да» — обновить
   замок на себя и продолжать. Старый замок мог зависнуть, мог идти прямо
   сейчас — без переспроса не лезем.
4. **После деплоя** (успешного или fail'нувшего):
   - Вернуть «🟢 Свободно».
   - Опционально дописать одну строку в журнал внизу файла
     (дата + кто + версия + что + результат + длительность).
   - Закоммитить (`chore(deploy): unlock — vX.Y.Z OK` или `... FAIL: <причина>`),
     запушить.

### Замок — не на коммит/push, а только на деплой

Коммитить и пушить в `main` можно когда угодно — замок защищает только этап
**выкатки на прод** (rsync + docker build + restart). Если фича готова и
запушена, но кто-то деплоит — подожди, потом катись своим деплоем.

### Аварийный сброс

Если замок «зависший» (стоит больше 1 часа, никаких сообщений в чате/git
истории, контакт с тем-кто-стоит потерян) — пользователь может явно
сбросить замок руками (`Статус: 🟢 Свободно` + строка в журнал
«principal force-unlock — <причина>»). Агенты сами по таймауту НЕ снимают
замок — только пользователь.

### Почему файл, а не лок-сервер

- Видим в `git status` / `git diff` — нельзя «не заметить»
- История изменений = аудит деплоев бесплатно
- Не нужен Redis / отдельный сервис
- Работает офлайн (агент видит state репо в любой момент)

Минус — race-условие между двумя одновременными commit'ами (оба видят
