...
## Правило 3.5 — Сохранность WB-данных: забрал → сохрани навсегда (КРИТИЧНО)

**Принцип:** всё, что мы хоть раз получили от WB, немедленно персистится в нашу БД
и НИКОГДА не теряется — ни при повторном синке, ни при деплое/ребилде, ни при
миграции. Причина: WB для многих данных — **не идемпотентный источник**: часть
данных он отдаёт лишь в узком окне и обратно не возвращает. Если мы их перетёрли
или не сохранили — они потеряны навсегда. Наша БД = единственный архив.

**Невосстановимые с WB данные (особый режим — копить, не терять):**
- **Воронка продаж** (`wb_funnel_daily`, Analytics v3 sales-funnel) — rolling-7,
  истории WB не отдаёт (v2 nm-report убит в 2025). Копить ежедневно.
- **Реклама fullstats** (`wb_ad_stats_daily`) — WB для throttled-продавца отдаёт
  нестабильно (дата может «обнулиться»).
- **Точечные снапшоты остатков** (`wb_stock_snapshot`) — состояние на момент.
- **Фин-отчёт** (`wb_report_detail`) — публикуется WB понедельно с лагом.

**Обязательные инварианты для синков (нарушение = потеря данных):**
1. **Не понижать/не затирать ненулевое нулём при повторном синке (FREEZE).**
   Перезаписывай дату/строку только если новые данные ≥ сохранённых, либо если
   сохранённого нет. Пример: `sync_ad_stats` freeze (DEV-058) — не понижает
   историч. ad spend нулём из пустого WB-ответа. НИКОГДА `delete(весь диапазон)`
   до вставки — удаляй только реально полученные даты, и только если они ≥ старого.
2. **Пустой/частичный ответ WB → skip, не delete.** Если WB вернул 0 строк за
   период — НЕ трогаем сохранённое (early-return). Уже соблюдается в orders/sales/
   stocks/ad_stats.
3. **Миграции — append/backfill, не replace.** Alembic не должен DROP/TRUNCATE
   таблицы с накопленными WB-данными. Только ADD COLUMN / backfill. Перед любой
   миграцией — pg_dump (Правило 3). Named volumes (`pgdata`) переживают
   деплой/ребилд (см. CLAUDE.md инвариант #17).
4. **Свежесть = мониторить.** `GET /api/sync/status` (per-tenant checkpoint).
   Застой обычно = WB throttle (429/cooldown), не наш баг; лечится on-demand
   refresh `POST /api/settings/sync/trigger {entity}` когда WB отвечает. Если
   checkpoint=ok, но данные не обновились — это БАГ (расхождение checkpoint↔данные).

**Доверие к данным (с какого числа считать полными) — фиксируй в задаче при сверках:**
- Заказы/Воронка — с момента, когда начали копить `wb_funnel_daily` (для Onyx —
  22.05.2026). Раньше — fallback `wb_orders` (без рассрочки/отменённых, занижено).
- Финансы (`wb_report_detail`) — по последнюю опубликованную WB неделю; текущая
  открытая неделя заполняется по мере публикации (понедельный лаг).
- Реклама — надёжна вперёд с момента включения freeze (06.06.2026).
